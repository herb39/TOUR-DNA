...
> 재검증하지 않았다.
>
> **2026-08-11 갱신**: 지원지역이 27개에서 로컬 DB 기준 **전국 SIGUNGU 255/255**로 완전히
> 확대됐다(`npm run audit:tourism-data` 최종 판정 PASS, ERROR 0). 유사지역 비교 모집단도 대상 지역
> 제외 최대 254곳까지 가능하다. 또한 이번 갱신에서 **다채널 마케팅 콘텐츠(아래 표)의 "LLM 미사용"
> 서술이 stale함을 확인해 정정한다** — 현재는 OpenRouter 무료 모델(`google/gemma-4-26b-a4b-it:free`)이
> 문구 생성에 쓰이고, 실패 시 기존 결정론적 rule 생성기로 자동 대체된다(상세는
> [docs/implementation-status.md](implementation-status.md) "2026-08-11 종합 갱신" 절 참고).
> 이번 갱신은 로컬 DB 기준이며 Production 배포 상태는 별도 확인이 필요하다.
...
| 다채널 마케팅 콘텐츠 | `/projects/[id]/plan`의 "홍보자료" 섹션 — 상단 포스터형·카드뉴스형 시각 미리보기(Phase 1) + 하단 채널별 텍스트 편집(제안서 요약/랜딩/Instagram/블로그/카드뉴스/숏폼/역할별 자료 생성·편집·복사, 채널 우선순위 역할별 정렬), 인쇄 화면 출력 | 저장된 실행안/Evidence를 검증된 context로 구성한 뒤 OpenRouter 무료 모델(`google/gemma-4-26b-a4b-it:free`)이 문구만 생성(실패 시 기존 결정론적 rule 생성기로 자동 대체) | `promoContent.test.ts`(29 — Phase 2 공통 채널 역할 반영 7건 포함), `promoContentAdapter.test.ts`(14), `promoContentService.test.ts`(16), `PromoContentEditor.test.tsx`(16+11), `promoPreview.test.ts`(13), `PromoPreviewPanel.test.tsx`(4), `PrintPage.test.tsx`(6), `promoContentFormat.test.ts`, `promoContentSchema.test.ts`, `promoLlmClient.test.ts`, `promoLlmGenerator.test.ts` | 2026-08-01 Production 브라우저(Playwright)로 홍보자료 생성→역할별 채널 순서→전체 복사→새로고침/재접속 유지까지 검증 완료(당시는 rule 생성기 기준). 2026-08-07 포스터·카드뉴스 미리보기와 공통 채널 역할별 관점 반영을 기존 Production 프로젝트(여행사/DMC·지자체/관광재단)의 실제 실행안 데이터로 재검증 | **구현·배포·검증 완료(rule 생성기 기준, 2026-08-01/07)** — 관련 커밋이 `origin/main`에 push되어 있고, `SelectedPlan.promoContent`·`StrategyResult` 차별화 필드 migration 모두 원격 Production Neon DB에 적용 완료. **배포 URL(`tour-dna.lib.lc`)에서 실제로 확인 가능.** 제안서 요약·랜딩·Instagram·블로그·카드뉴스 5개 공통 채널도 역할(여행사/DMC·지자체/관광재단·축제 기획자)에 따라 강조점과 문장 구성이 실제로 달라진다 — DNA 5축 원시 점수와 대표 코스·근거 데이터 같은 사실 값은 역할과 무관하게 동일하게 유지된다. **2026-08-10~11에 로컬 구현으로 OpenRouter LLM 문구 생성 오버레이가 추가됐다**(정량 분석·전략·POI·실행안은 여전히 LLM이 전혀 관여하지 않고, 검증된 context를 LLM이 실무용 다채널 문구로 변환하는 구조) — 이 LLM 경로는 아직 Production에 배포·검증되지 않았고, 대표 프로젝트(강릉/경주/제천)의 현재 저장된 홍보자료는 LLM이 아니라 rule 생성기 결과(`generatedBy: "rule"`)다. 무료 모델의 timeout·429가 반복 관찰돼 라이브 시연에서는 실시간 생성 버튼에 의존하지 않는 것을 권장한다. 상세: [docs/implementation-status.md](implementation-status.md)의 "Production 실사용 검증 및 대표 시나리오 완성(2026-08-01)" 절, "홍보자료 포스터·카드뉴스 미리보기 Phase 1(2026-08-07)" 절, "홍보자료 공통 채널 역할별 관점 반영 Phase 2(2026-08-07)" 절, "2026-08-11 종합 갱신" 절 |
...
- 다만 아래는 심사 시 바로 드러날 수 있는 리스크:
  ...
  4. (2026-08-13 갱신) 대전 DNA는 유성구 지표라는 행정범위를 라벨 하나로만 알려주던 문제 자체는 근본
     원인(`Region.name`이 "대전광역시"로 잘못 저장돼 있던 것)을 바로잡아 해소했다 — 이제 시/군/구
     드롭다운에 "유성구"가 정확히 나온다(과거 서술: 대전 DNA는 유성구 지표, 추천 POI는 대전 전체라는
     행정범위 불일치가 화면에 라벨 하나로만 표시됨(`"대전광역시 (DNA 지표는 유성구 기준)"`,
     [regionQueries.ts])). 다만 근거 패널에 코호트 수(N)나 정규화 범위가 표시되지 않는다는 지적은
     여전히 유효하다.

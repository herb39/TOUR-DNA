...
신규 20곳 모두 `PoiRelation`(연계 축 관계 데이터)이 전혀 없다 — 이는 결측이 아니라, 애초에 `Poi
Relation`이 seed 수작업 큐레이션 전용이라 신규 지역에 대해 만든 적이 없기 때문이다(위 "연계 축 관계
데이터 CURATED" 항목 참고). 그 결과 신규 지역은 기존 7곳 중 일부보다 오히려 축 근거 구성이 더 단순
(POI API 근거만, CURATED 관계 근거 없음)하다.

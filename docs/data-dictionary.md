...
- **상대점수 0의 의미**: DNA 5축 점수는 같은 행정단위(SIGUNGU) 코호트 안에서 min-max 정규화한
  ...
  상대적으로 0점)는 화면에서 서로 다른 배지로 구분한다. **지역 확장(3→7→27→255곳) 영향**: 코호트가
  ...
  **2026-08-11 갱신**: 로컬 DB는 전국 SIGUNGU 255개까지 확장됐고, Demand(`tarSvcDemIxVal`/
  `touResDemIxVal`)·Spend(`tarExpDsIxVal`) 두 축은 코호트 확장에 더해 log1p(raw)→min-max 정규화도
  적용해 극단값 민감도를 완화했다(`docs/scoring-model.md` 참고, Stay/Diversity는 여전히 선형 min-max).
- **유사지역 비교의 모집단 제한**: 유사지역 비교는 전국이 아니라 **현재 지원하는 SIGUNGU 지역만의
  모집단** 안에서 계산한다(2026-08-07 당시 27곳 — 위 지역 표는 그 시점 기록). 대상 지역을 제외하면
  후보는 (전체 지원 지역 수 − 1)이다 — 계산(RMS 거리 + POI 구성 거리, `regionSimilarity.ts`) 자체는
  정확히 재현 가능하다. 화면은 `candidatePoolSize`(대상 지역을 제외한 후보 수)를 그대로 표시하고,
  임계값(현재 10곳) 미만이면 참고용 안내를 추가로 보여준다 — 27곳 확장 이후로는 후보가 항상 10곳을
  넘어 이 안내가 표시되지 않는다. **2026-08-11 갱신**: 로컬 DB 기준 지원 SIGUNGU는 255곳으로 늘었고,
  유사지역 후보 모집단도 최대 254곳까지 늘었다(대상 지역 1곳 제외).

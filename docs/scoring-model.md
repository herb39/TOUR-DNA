...
- **내부 분석점수와 사용자 표시지수는 다른 계층이다(2026-08-07)**: 위 정규화·가중치·전략 계산은 전부
  이 절의 내부 0~100 분석점수를 그대로 쓴다. 화면에 그리는 숫자만 `dnaDisplayScore.ts`가 10~90
  범위로 선형 압축해 보여준다(0/100이 절대평가처럼 보이는 것을 완화하는 표시 전용 변환, 순위·비율
  관계는 그대로 유지). 이 변환은 저장하지 않고 화면 렌더링 시점에만 적용한다.
...
- 체크리스트/KPI/위험은 전략 템플릿에 미리 정의된 목록(`kpiTemplates`, `riskTemplates`)을 초기값으로
  가져온다. **2026-07-22부터 이 세 목록도 실행안 편집기에서 자유롭게 추가/삭제할 수 있다** — 공공데이터
  API에는 체크리스트/위험/KPI에 대응하는 값 자체가 없으므로(전략 템플릿 기반 고정 문구), 여기엔 "API
...
- 실행안 편집기(`PlanEditor.tsx`)에서 코스를 자유롭게 고칠 수 있다 — 같은 날짜 안 순서 변경/삭제/다른
  ...
  — 순서 변경은 방문 순서(따라서 이동거리 재계산)만 바꾸고 시간은 건드리지 않으며, 시간은
  `<input type="time">`으로 직접 수정한다. 하루에 담을 수 있는 장소 수는 제한하지 않는다.
- 인쇄 화면(`/print`)도 체크리스트/위험/KPI를 전부(개수 제한 없이) 보여주도록 맞췄다(2026-07-22 이전에는
  체크리스트를 5개까지만, 위험은 아예 표시하지 않았다 — 편집 가능해진 이상 저장된 내용을 빠짐없이
  보여주는 게 맞다고 판단).

# 구현 상태 (2026-08-18 갱신 — P1-2b 축제 Anchor 코스 고정 연결 반영)
...
| 5. 특별한 기획 계기 | 부분 완료(P1-2b, 로컬 구현) | 공식 후보 조회 → 프로젝트 Anchor 확정 저장·변경·삭제 → 실행안에서 사용자가 명시적으로 승인한 정확한 시각의 Anchor만 지정 일차에 고정. 기존 POI 보존·Anchor snapshot 저장·stale 저장 거부까지 연결 | Production migration 적용, 이후 P1-2c에서 Anchor 전후 관광·식음·숙박 후보 연결 |
...
- 9단계 기준으로 완전 완료 5개(1·2·6·7·9), 대부분 완료 1개(3), 부분 완료 3개(4·5·8)다. 5단계는 P1-2b까지 진행됐고, Anchor 전후 자원 연결은 P1-2c로 남아 있다.
...
- P1-2a migration `20260818000000_add_project_anchor`는 확인된 로컬 PostgreSQL(`localhost:5432/tour_dna_local`)에만 적용했다. Production Neon에는 migration/apply하지 않았다. 구 스키마 Production에서도 `ProjectAnchor` 테이블이 없으면 분석·실행안 화면은 `UNAVAILABLE` 안내로 계속 열리고, 확정 저장과 Anchor가 포함된 코스 저장만 보류되도록 fallback을 유지한다. 따라서 Production 기준 P1-2b도 migration 승인·적용 전까지 “부분 완료”다.
- POI 큐레이션은 운영자 화면 없이 런타임 보조 계층으로만 유지한다. 큐레이션 테이블이 없는 환경에서는 기본 POI 조회로 폴백하며, 이번 문서 갱신에서 Neon DB에는 접근하지 않는다.

### 다음 작업 순서

1. **P1-2c 축제 Anchor 전후 연결**: 이미 코스에 명시적으로 고정된 Anchor 전후에 관광·식음·숙박 후보를 연결한다. 자동 추천·전략 점수 변경 없이 기존 편집·검증 흐름을 재사용한다.
2. **핵심 테마 슬롯과 후보 재정렬 강화**: 선택한 전략 테마가 코스에서 일정 비중을 차지하도록 후보 풀 점수·자동 초안·편집 후 재정렬을 한 흐름으로 묶는다.
3. **검증 품질 확장**: 구조화된 접근성·반려동물 근거를 활용해 검증하고, POI 유형별 권장 체류시간과 이동거리 억제를 적용한다. 외지인 방문 가치/대표성 평가는 장기 과제로 둔다.

> 이번 개발 단위에서 **“확정 Anchor의 명시적 코스 고정 연결(P1-2b)”**을 로컬 코드·저장 검증·지도·인쇄·테스트까지 구현했다. 다음 개발 단위는 **“Anchor 전후 관광·식음·숙박 연결(P1-2c)”**이며, Production Neon migration은 별도 승인 후 적용한다. 운영자 화면은 제품 범위에 포함하지 않는다.

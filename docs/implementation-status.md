# 구현 상태 (2026-08-18 갱신 — P1-2a 축제 Anchor 프로젝트 저장 반영)
...
| 5. 특별한 기획 계기 | 부분 완료(P1-2a) | 공식 후보 조회 → 날짜·일차·시간 조건 명시 → 프로젝트 Anchor 확정 저장·변경·삭제. 로컬 DB에서는 다른 브라우저에서도 공유됨 | Production migration 적용, 이후 P1-2b에서 코스 고정 일정·전후 관광/식음/숙박 연결 |
...
- 9단계 기준으로 완전 완료 5개(1·2·6·7·9), 대부분 완료 1개(3), 부분 완료 3개(4·5·8)다. 5단계는 P1-2a(프로젝트 Anchor 확정 저장)까지 진행됐고 코스 연결은 아직 하지 않았다.
...
- 축제 연계는 P1-1의 한국관광공사 `searchFestival2` 후보를 P1-2a에서 서버 확정 흐름으로 확장했다. 서버는 확정 시 공식 후보를 다시 조회해 `contentid`·행사 기간·지역을 검증하고, 사용자가 직접 고른 연계 날짜·여행 일차·시간 조건과 최소 스냅샷만 `ProjectAnchor`에 저장한다. 행사 시각이 API에 없으면 `UNCONFIRMED`로 보존하며 시각을 추정하지 않는다. 기존 `localStorage`는 미확정 UI 선택의 호환 fallback일 뿐, 서버 Anchor가 있으면 서버 상태가 항상 우선한다. 저장·변경·삭제는 프로젝트 접근 쿠키와 `Project.updatedAt` optimistic concurrency를 사용하고 코스·전략 점수에는 아직 연결하지 않는다.
- P1-2a migration `20260818000000_add_project_anchor`는 확인된 로컬 PostgreSQL(`localhost:5432/tour_dna_local`)에만 적용했다. Production Neon에는 migration/apply하지 않았다. 구 스키마 Production에서도 `ProjectAnchor` 테이블이 없으면 분석·실행안 화면은 `UNAVAILABLE` 안내로 계속 열리고, 확정 저장만 보류되도록 fallback을 넣었다. 따라서 Production 기준 P1-2a는 migration 승인·적용 전까지 “부분 완료”다.
- POI 큐레이션은 운영자 화면 없이 런타임 보조 계층으로만 유지한다. 큐레이션 테이블이 없는 환경에서는 기본 POI 조회로 폴백하며, 이번 문서 갱신에서 Neon DB에는 접근하지 않는다.

### 다음 작업 순서

1. **P1-2b 축제 Anchor 코스 연결**: 이미 확정된 Anchor를 사용자가 별도로 승인했을 때만 고정 일정으로 넣고, 전후 관광·식음·숙박 후보를 연결한다. 자동 삽입·전략 점수 변경은 하지 않는다.
...
> 이번 개발 단위에서 **“선택 Anchor의 프로젝트 저장·명시적 확정·변경·삭제(P1-2a)”**를 로컬 schema와 테스트까지 구현했다. 다음 개발 단위는 **“확정 Anchor의 코스 연결(P1-2b)”**이며, Production Neon migration은 별도 승인 후 적용한다. 운영자 화면은 제품 범위에 포함하지 않는다.

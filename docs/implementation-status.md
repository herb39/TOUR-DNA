...
신규 프로젝트의 `분석 시작` 클릭부터 분석 화면의 핵심 DNA 5축이 보일 때까지와, 저장된
프로젝트의 `/analysis` 재진입부터 같은 핵심 정보가 보일 때까지를 분리해 로컬 QA에서 측정했다.
기준은 브라우저 체감 시간과 서버 계측을 함께 사용했으며, `ANALYSIS_TIMING=1`일 때만 단계 로그가
남는다. 기본 실행·Production 로그에는 계측 출력이 없다.

### 측정 결론

- 기존 분석 재진입은 경주 0.48~0.50초(첫 개발 컴파일 포함 2.62초), 청주 0.49~0.54초로
  신규 분석과 다른 경로였다. 재진입은 저장된 `AnalysisResult`와 유사지역 비교 스냅샷을 읽는다.
- 수정 전 신규 분석은 경주 5.62~5.94초(평균 5.76초), 청주 5.61~5.74초(평균 5.65초)였다.
  서버 계측에서 유사지역 비교 프로필 생성이 약 4.9초로 신규 분석 총시간의 대부분을 차지했다.
- `fetchRegionComparisonProfiles()`가 전국 SIGUNGU 255개 지역 각각에 대해 같은 8개 metric cohort를
  반복 조회하고 있었다. 지역 간 계산은 이미 병렬이었으므로, 병목은 순차 지역 계산이 아니라
  요청 한 번 안의 동일 DB 조회 반복이었다.
- 유사지역 계산 산식, DNA·전략 계산, 저장 결과는 바꾸지 않고 요청 범위 `Promise` 공유만 적용했다.
  수정 후 신규 분석은 경주 1.63~1.96초(평균 1.78초), 청주 1.64~1.77초(평균 1.69초)로
  약 69~70% 감소했다. 기존 분석 재진입은 약 0.48~0.50초로 유지됐다.

### 대표 측정값

| 시나리오 | 지역 | 수정 전 3회(ms) | 수정 후 3회(ms) | 평균 변화 |
| --- | --- | --- | --- | --- |
| 신규 분석 | 경주 | 5,941 / 5,723 / 5,617 | 1,956 / 1,758 / 1,631 | 5,760 → 1,782ms |
| 신규 분석 | 청주 흥덕구 | 5,735 / 5,610 / 5,611 | 1,768 / 1,649 / 1,643 | 5,652 → 1,687ms |
| 기존 재진입 | 경주 | 2,619 / 480 / 503 | 2,456 / 492 / 477 | 개발 컴파일 제외 warm 492 → 485ms |
| 기존 재진입 | 청주 흥덕구 | 522 / 536 / 491 | 491 / 491 / 495 | 516 → 492ms |

첫 번째 경주 재진입 값은 개발 서버 컴파일을 포함한 cold-ish 요청이다. 제품 성능과 개발 서버
컴파일 시간을 섞어 해석하지 않는다.

### 단계·DB·외부 API 계측

수정 전 대표 신규 요청의 서버 계측은 `project-load 3ms`, `active-dataset 2ms`, `dna-input
63ms`, `DNA 계산 1ms`, `strategy 계산 31ms`, `유사지역 프로필 4,915ms`, `유사지역 계산
1ms`, `persist 811ms`, 총 `5,164ms`였다. 수정 후에는 경주 대표 요청에서 `dna-input 24ms`,
`DNA 계산 1ms`, `strategy 계산 35ms`, `유사지역 프로필 970ms`, `유사지역 계산 1ms`,
`persist 944ms`, 총 `1,146ms`였고, 청주 대표 요청은 유사지역 프로필 `1,087ms`, 총 `1,210ms`였다.
benchmark와 preLaunch 순수 계산은 모두 0~1ms로 병목이 아니었다.

유사지역 프로필 생성 내부의 코드 호출 기준 DB 조회는 수정 전 `1 + 255 × 12 = 3,061회`였다.
이 중 동일한 8개 metric cohort가 `255 × 8 = 2,040회` 반복됐다. 수정 후 해당 cohort는
`8회`만 조회하고 지역별 POI·지역 조회는 유지해 `1 + 8 + 255 × 4 = 1,029회`로 줄었다.
이는 Prisma 서비스 호출 기준이며, relation `include`가 내부에서 생성하는 저수준 SQL 문장 수와는
구분한다. 축별 5개 cohort는 기존대로 순차, 방문자 3개 cohort는 기존대로 병렬이며, 255개 지역
프로필 계산의 병렬 구조와 산식은 유지했다.

분석 페이지 재진입에서는 다음을 확인했다.

- `TOUR_FESTIVAL/searchFestival2`: 분석 페이지마다 1회, warm latency 약 87~148ms, 초기 cold-ish
  관찰 최대 2,563ms. `timeoutMs=8,000`, `maxRetries=2`, 관찰된 retry 0회·timeout 0회.
- 신규 분석 계산 자체는 외부 API를 호출하지 않고 로컬 DB의 승인된 dataset/snapshot을 사용한다.
  분석 페이지의 축제 후보 조회 1회는 수정 전후 동일하다.
- 외부 축제 조회에는 기존 persistent cache가 적용되지 않아 `NOT_APPLIED`이며, 유사지역 비교는
  기존 프로젝트 재진입에서 `regionComparisonSnapshot` cache hit, 신규 분석에서는 저장할 snapshot을
  계산한다. 이번 수정은 새 cache subsystem을 만들지 않고 요청 범위의 pending 조회만 공유한다.

### 결과 동일성·보존 범위

수정 전후 같은 조건으로 생성한 경주·청주 프로젝트를 로컬 PostgreSQL에서 읽어 비교했다. 두 지역
모두 DNA 5축 값, `overallDataMode`, `liveAxisCount`, 전략 3안의 이름·순위·`totalScore`·
`templateId`가 일치했다. 후보 ranking, hard filter, 자동 코스, benchmark 의미, provenance,
외부 API 계약은 변경하지 않았다. Production Neon에는 접근하지 않았고, 보호 파일
`scripts/delete-route-geometry-verification-project-3.mts`와 `tour_dna_local.dump`도 수정·삭제·stage하지 않았다.

### 로컬 검증

- 관련 단위 테스트 54개 통과, 전체 단위 테스트 1,693개 통과.
- 기본 30초 `core-flow.spec.ts`: 8개 통과.
- 기본 timeout 전체 `npm run test:e2e:local`: 38개 통과, 2개 선택적 skip.
- `typecheck`, `lint`, `build`, `git diff --check` 통과.
- 입력 화면은 제출 중 버튼이 `분석 중...`으로 바뀌고 비활성화된다. 별도 skeleton 없이 입력 화면이
  유지되며, 수정 후 대표 신규 분석은 2초 안팎이다. 남은 10초 이상 외부 대기 시 시각 피드백은
  후속 UX 과제로 남긴다.

## 2026-08-26 갱신 — 분석·전략 화면 정보 계층과 수치 의미 정리

공모전 심사위원이 분석 결과를 처음 봤을 때 `지역의 강점·개선 축 → 추천 전략 3안 → 숫자와
근거의 의미`를 10~15초 안에 파악할 수 있도록 사용자 노출 순서와 라벨을 정리했다. DNA·전략
산식, 정규화, 전략 순위·tie-break, benchmark, provenance, DB·API 계약은 변경하지 않았다.

### 변경 전후 정보 계층

- 기존 1280×720 기준 경주 화면은 헤더 뒤에 축제 Anchor(약 y=532), 사업 사전검증(약 y=825)이
  먼저 나오고 DNA 5축은 약 y=1,339, 전략 3안은 약 y=2,058에서 시작했다. 청주도 DNA 약
  y=1,323, 전략 약 y=2,042로 같은 문제가 반복됐다.
- 상단에 `데이터 최신성`과 `핵심 진단`을 두고 강점·개선 축을 짧은 배지로 노출했다. DNA 5축과
  레이더를 전략보다 앞에 배치하고, 전략 이름 3안과 `전략 3안 확인하기 →`를 상단에 남겼다.
- 전략 비교와 카드가 먼저 이어지고 지도 다음에 역할별 해석·축제 Anchor·사전검증·핵심 지표·
  입력 조건·유사지역/기회 상세를 배치했다. 상세 `details`는 기본 접힘을 유지해 원문·근거·
  benchmark가 첫 진입을 가리지 않는다. 현재 경주·청주 데스크톱 검증에서 전략 제목은 약
  y=987, 카드 제목은 약 y=1,207로 확인했다.
- 모바일 390×844에서는 DNA 표시 label·전략 CTA가 유지되고 문서 루트의 가로 overflow가 없다.
  레이더의 스크린리더 대체 표도 별도 숨김 컨테이너 안에 두어 접근성 정보와 화면 폭을 함께 보존했다.
  인쇄 화면의 기존 보고서 내용과 순서는 바꾸지 않았다.

### 사용자에게 보이는 숫자 계약

| 화면 표기 | 실제 값/범위 | 의미와 주의점 |
| --- | --- | --- |
| `DNA 상대지수` | `axisDisplayScoreByAxis`의 10~90 | `analysisResult`의 내부 DNA 축 점수(0~100)를 `toDisplayDnaScore()`로 화면에만 선형 압축한 표시값. 강점·개선 판정과 전략 계산에는 사용하지 않는다. |
| `원값` | `EvidenceRow.rawValue` | 출처 API/DB가 제공한 해당 지표의 원자료 값. 지표별 단위·범위가 다르므로 DNA 상대지수와 직접 비교하지 않는다. |
| `원천 지표 정규화값` | `EvidenceRow.normalizedValue` | 개별 원천 지표를 비교 코호트·변환 규칙에 따라 정규화한 근거 값. 여러 지표를 축에서 종합하기 전 값이며 DNA 표시값과 동일한 숫자가 아니다. |
| `전략 적합도` | `StrategyResult.totalScore` 0~100 | 수요 30%·공급 20%·계절 20%·타깃 15%·실행가능성 5%·역할 10%을 반영한 입력 조건과 전략 템플릿의 궁합. 성공 확률·방문객 수·매출 예측이 아니다. |
| `데이터 최신성` | `overallDataMode`와 `liveAxisCount` | `LIVE`는 `5개 축 모두 최신 데이터`, `HYBRID`는 `n/5축 최신 데이터`, `SNAPSHOT`은 `저장된 데이터 사용`으로 표시한다. 옆의 기준월은 근거 데이터의 기준월 요약이다. |
| 관광 핵심 지표 | 방문자수·증감률·체류/소비 지수의 원값 요약 | DNA 점수나 전략 적합도가 아니다. 체류·소비 지수도 화면에서 비교군 내 상대 강도로 설명한다. |
| 유사지역 비교 | 비교 지역의 표시 DNA 지수와 차이 | benchmark 참고용 상대 비교이며 인과관계나 성과 예측이 아니다. |

이전의 `점수`, `정규화값`, 역할 요약의 `약점(28점)`처럼 단위가 생략돼 숫자가 서로 다른 계층인데도
같아 보이던 표현을 각각 `DNA 상대지수`, `원천 지표 정규화값`, `전략 적합도`로 구분했다. `근거
보기`를 열면 원값·원천 지표 정규화값·단위·기준월·출처·반영 규칙을 함께 확인할 수 있고, DNA
카드의 표시값은 해당 지표들의 종합 표시값이라는 안내를 제공한다.

### 이번 범위의 보존 원칙

- `src/lib/domain/dna.ts`, `src/lib/domain/normalize.ts`, `src/lib/domain/strategy.ts`의 계산·
  정규화·순위·tie-break를 변경하지 않았다.
- 후보 ranking·hard filter·자동 코스·Anchor/PET/ACCESSIBILITY·체류시간·지도 route·Course
  Studio·benchmark 산식·Prisma schema·DB/API 호출을 변경하지 않았다.
- 로컬 QA fixture만 사용했고 Production Neon에는 접근하거나 쓰지 않았다.

### 로컬 검증 요약

- 신규 `e2e/analysis-information-hierarchy.spec.ts`에서 경주·청주 1280×720 핵심 순서, 숫자 label,
  근거 표, 상세 기본 접힘을 확인했다.
- 동일 E2E에서 390×844 모바일 overflow·전략 CTA와 인쇄 화면의 핵심 지표·전략·인쇄 버튼 숨김을
  확인했다. 레이더 접근성 대체 표가 모바일 폭을 침범하지 않는 회귀도 포함한다.
- 단위 테스트에는 DNA 표시값/개별 근거 정규화값/전략 적합도 label과 PRIMARY 섹션 순서 검증을
  추가했다. 관련 E2E는 `4 passed`, 전체 로컬 E2E 기본 제한 실행은 `34 passed / 4 failed /
...
공모전 심사위원이 실행안 화면에 처음 들어왔을 때 `선택한 전략 → 자동 생성된 코스 →
직접 편집 → 실시간 검증 → 저장·확정`을 10~15초 안에 파악할 수 있도록 표시 순서와 기본
접힘 상태를 정리했다. 코스 생성·추천·검증·저장 로직과 데이터 계약은 변경하지 않았다.

- 상단에 선택 전략, 일정 일수·장소 수, 품질 요약, 5단계 실행 흐름을 한 줄의 요약 영역으로
  표시한다. 헤더에는 기간·테마·여행 조건을 작은 칩으로 남겨 현재 기획 맥락을 빠르게 확인할
  수 있게 했다.
- 실행안의 핵심인 `일자·시간대별 코스`와 지도는 상품 기획 상세·사전검증 리포트보다 앞에
  배치했다. 사업 사전검증은 편집기 아래로 이동했고, 역할별 긴 설명은 기존 전략 상세 안으로
  넣어 첫 화면을 압축했다.
- 코스 품질 패널은 경고 수와 저장 가능 여부를 상단에 남기고, severity별 상세 설명은 기본
  접힘으로 바꿨다. 경고가 있어도 저장할 수 있다는 문구를 명시했다.
- 데스크톱 우측에는 `저장·확정` 상태와 저장 버튼을 먼저 배치했다. 일반 추천 후보와 축제
  Anchor 연계 후보의 목록은 기본 접힘으로 두고, 개수·용도·`후보 목록 열기` 또는
  `연계 후보 목록 보기`로 존재와 조작 방법을 알 수 있게 했다. 기존 후보 추가·삭제·Drag &
  Drop 동작은 그대로 유지했다.
- 모바일에서는 고정 저장 바를 추가하지 않고 기존 저장 영역을 유지했다. 상단 흐름에서 저장
  단계를 먼저 안내하고, 코스 편집 뒤 저장 영역으로 자연스럽게 이어지도록 했다. 인쇄용
  레이아웃과 전체 보고서 정보는 보존했다.

### 이번 변경에서 보존한 계약

추천 후보 ranking, hard filter, 자동 코스, 전략 점수·DNA 계산, 실시간 품질 계산, PET/
ACCESSIBILITY 의미와 evidence 표시, Festival Anchor ranking·확정·검증, 유형별 체류시간,
지도 route, Prisma schema, DB 데이터 및 Production Neon 접근은 변경하지 않았다. 변경 파일은
화면 표시 순서·접힘 기본값·접근성용 검증과 관련 테스트·문서로 한정했다.

### 로컬 검증 요약

- 1280×720 일반 프로젝트와 Festival Anchor 프로젝트: 상단 흐름·코스·지도·저장·확정 영역이
  첫 viewport에서 확인되고, 후보 목록과 상세 품질 설명은 기본 접힘이다.
- 390×844 모바일: 상단 흐름·코스·지도 순서와 가로 overflow 없음을 확인했다. 저장 영역은
  고정 바 없이 기존 코스 편집 흐름 뒤에서 찾을 수 있다.
- `tests/unit/PlanEditor.test.tsx`, `tests/unit/PreLaunchValidationSection.test.tsx` 관련
  단위 테스트 81개와 새 정보 계층 E2E 3개가 통과했다. Anchor 저장 회귀도 단독 재실행에서
  1개 통과·선택적 fixture 2개 skip으로 확인했다.


## 2026-08-26 갱신 — 공모전 심사위원 관점 통합 브라우저 QA

현재 기능을 처음 접하는 심사위원의 관점에서 로컬 `http://localhost:3000`을 처음 화면부터
분석·전략·실행안·인쇄까지 실제로 사용했다. Production Neon에는 접근하지 않았고, 로컬 QA
fixture와 신규로 생성한 로컬 프로젝트만 사용했다.

### 실제 시나리오

- 시나리오 A는 홈의 `새 관광상품 기획`에서 시작했다. 경주 대표 카드를 적용한 뒤 경주시·2026년
  10월·지자체/관광재단·내국인·40/50대·체류 확대·2박 3일·대중교통·문화·역사+웰니스·뚜벅이 조건을
  확인하고 직접 수정해 분석을 생성했다. 분석 `4/5축 최신 데이터`, DNA 5축, 근거 패널, 전략 3안,
  전략 선택, 실행안, 지도, 저장·재로드, 인쇄까지 이어지는 신규 프로젝트가 정상 생성됐다.
- 시나리오 A의 실행안에서 같은 날 재정렬과 날짜 간 Drag & Drop을 실제 포인터 동작으로 수행했고,
  추천 후보 추가·삭제·체류시간 수정·저장·재로드를 확인했다. 유형별 기본 체류시간은 화면 입력에서
  관광지 `90분`, 음식 `60분`으로 보였고 사용자 수정값도 저장 후 유지됐다. 시간 순서가 어긋난 경우
  `시간표와 이동시간 불일치`가 실시간 품질 패널에 나타났다.
- 시나리오 A의 축제 후보 `신라문화제`를 선택해 2026-10-10·2일차·오후 기획 블록을 지정하고
  Project Anchor로 확정했다. 공식 행사 시각이 없는 경우에는 정확한 시각을 지어내지 않고 기획
  시간대만 저장하는 현재 정책도 확인했다.
- 시나리오 B는 기존 로컬 QA 프로젝트 `[QA E2E] 대전 축제 Anchor`와 `[QA PET 사용자] 경주`를
  사용했다. 대전 Anchor를 공식 `contentId=4098134` 행사 `유성온날`(`2026-08-14~2026-12-11`,
  유성구 온천로 89)로 정합화한 뒤 코스에 고정·저장·재로드했고, 행사 전·식사·행사 후·숙박 후보
  12개가 표시됐다. 행사 전 후보를 추가했다가 삭제해 원상복구했으며, PET·ACCESSIBILITY 조건은
  공식 확인·조건부·미확인 상태와 상세 evidence를 함께 확인했다.

### 통합 QA 판정

- P0: 발견하지 못했다. 신규 프로젝트 생성, 분석, 전략 선택, 실행안, 저장, 재로드, 인쇄가 막히지
  않았고 로컬 오류 화면이나 Production DB 접근도 발생하지 않았다.
- P1: 제품 코드 기준으로 즉시 수정할 결함은 발견하지 못했다. 축제 Anchor 전후 후보는 Anchor를
  코스에 반영한 뒤 저장·재로드하면 정상 재계산된다. 저장 전에는 서버 재계산을 하지 않는 현재
  계약이 화면 안내와 함께 동작한다.
- P2: 분석·실행안의 정보량, 조건 advisory badge 반복, 홈의 최근 프로젝트 목록 밀도는 여전히
  심사위원에게 한 번 더 설명해야 하는 영역이다. 기능 차단은 아니므로 이번 QA에서는 화면 구조를
  다시 설계하지 않았다. 이전 대전 QA fixture의 `2025` 행사명·`2026` 행사일 불일치는 공식
  `searchFestival2` 확인 결과로 교정했고, 이제 행사명·기간·지역·주소·좌표·sourceSnapshot이
  동일한 공식 후보를 가리킨다.
- POLISH: Anchor 후보를 저장·재로드 전에 즉시 다시 계산하지 않는 점, 인쇄 화면의 긴 보고서 밀도,
  첫 화면의 최근 프로젝트 표 밀도를 후속 개선 후보로 남겼다. PET/ACCESSIBILITY 미확인은 이용
  불가로 해석하지 않는 안내와 접힌 상세 구조는 현재 상태를 유지한다.

이번 라운드는 공식 Festival API 조사 결과를 로컬 QA fixture setup에 반영하고, setup이 같은
fixture를 두 번 실행해도 `ProjectAnchor`를 반복 갱신하지 않도록 idempotency를 보완했다. `sourceSnapshot`
에는 행사명·기간·주소·좌표·공식 sourceId를 저장하고, setup이 전달한 행사명·공식 기간을 Anchor E2E에서
확인한다. 후보 조사는 대전 유성구 2026년 9월, 세종 2026년 10월, 제천 2026년 12월의
`searchFestival2` 조회 3회로 한정했으며, 상세 API·전국 행사 대량 조회는 수행하지 않았다.
Production Neon·스키마·DNA·ranking·PET/ACCESSIBILITY 로직은 변경하지 않았다.

## 2026-08-26 갱신 — Course Studio 저장 상태 정확화

실행안 편집 화면의 저장 안내를 실제 데이터 상태와 서버 응답에 맞게 분리했다. 기존에는 최초
진입 시 `isDirty=false`인 것만으로 `모든 변경사항이 저장되었습니다.`가 표시되어, 사용자가 실제
저장 성공으로 오해할 수 있었다.

- 최초 진입·원복 상태는 `CLEAN`(`현재 저장된 내용과 같습니다.`), 실제 payload가 baseline과
  달라진 동안은 `DIRTY`, 요청 진행 중은 `SAVING`, 실제 성공 응답을 받은 직후에만 `SAVED`를
  표시한다. 저장 실패는 `ERROR` 안내를 보여주고 저장 대상은 계속 `DIRTY`로 남겨 재시도할 수 있다.
- dirty 비교 대상은 서버 저장 payload인 상품명·콘셉트·메모·KPI 메모·일정·운영 체크리스트·위험·
  KPI다. 상세 접기, 후보 날짜 임시 선택, 필터·포커스·후보 재정렬 같은 UI-only state는 제외했다.
- baseline은 최초 진입값이 아니라 마지막 실제 성공 응답의 payload로만 갱신한다. 저장 A가 진행되는
  동안 B를 편집하면 A의 응답으로 B를 덮어쓰지 않고 B를 `DIRTY`로 유지한다. 서버 최종 DB 저장
  예외도 일반 오류 응답으로 반환해 성공 문구가 표시되지 않는다.
- 후보 추가/삭제, 순서·날짜 이동, 시간·체류시간, 텍스트·숙박·체크리스트/KPI/위험/메모 편집을
  저장 상태 계약에 포함했다. 자동저장·이탈 차단·ranking·DNA·전략 점수·PET/ACCESSIBILITY·
  route·스키마·Production Neon은 변경하지 않았다.
- `PlanEditor` 저장 상태 단위 테스트 73개와 대표 로컬 QA E2E에 최초 진입·편집·성공·재로드·모바일
  확인을 반영했다. 실제 QA 환경변수가 없는 E2E는 기존 정책에 따라 skip된다.

## 2026-08-26 갱신 — POI 유형별 기본 체류시간 감사·적용 완료

자동 초안과 Course Studio 후보 추가 경로를 감사한 결과, 일반 POI가 모두 `60분`으로 생성되고
숙박만 `0분`, 사용자 확정 시각이 있는 Festival Anchor만 실제 시작·종료 시각 차이를 사용하는
계약이었다. 경주 문화역사·청주 흥덕구 자연웰니스·대전 유성구 축제 Anchor 대표 저장 코스도
저장된 비숙박 POI가 모두 60분으로 남아 있어 박물관·역사 명소에는 비현실적으로 짧고, 유형별
차이를 반영하지 못하는 문제가 확인됐다.

- `recommendedPoiStayMinutes()` 순수 계약을 추가해 자동 초안·후보 추가·Anchor 전후 연계 후보가
  같은 기본값을 사용한다. `ATTRACTION=90`, `VE07 박물관·전시=120`, `EXPERIENCE=120`,
  `LS 레저=180`, `FOOD=60`, `CAFE=45`, `SHOPPING=60`, `FESTIVAL=120`, `LODGING=0`,
  미분류 fallback은 `60분`이다.
- Festival Anchor 자체는 기존처럼 사용자가 확정한 정확한 시작·종료 시각 차이를 그대로 유지하고,
  사용자 수정 체류시간은 재정렬·날짜 이동·저장·재로드에서 덮어쓰지 않는다. 기존 저장 코스는
  일괄 마이그레이션하지 않아 사용자가 직접 수정한 값과 기존 legacy 값은 보존된다.
- 자동 초안의 시간 계산·일일 적합성 계산만 새 기본값을 사용하며, 품질 점수·DNA·전략 점수·후보
  ranking·PET/ACCESSIBILITY·route·스키마·Production Neon은 변경하지 않았다.
- 대표 코스를 메모리에서 새 계약으로 재생성한 비교에서는 청주 `300+120분 → 240+180분`,
  대전 `240+120분 → 240+300분`, 경주 `240+240+120분 → 300+330+150분`으로 바뀌었고,
  저장 코스 자체는 변경하지 않았다. 이는 새 초안 생성 시의 영향 확인용 시뮬레이션이다.
- 단위 테스트 128건과 유형별 저장·날짜 이동·재로드 E2E 8건을 통과했다. 이번 단계에서 추가한
  E2E의 `130분`은 화면 입력 `step=10` 제약을 따르는 사용자 수정값이다.
...
- 9단계 기준으로 완전 완료 5개(1·2·6·7·9), 대부분 완료 1개(3), 부분 완료 3개(4·5·8)다. 5단계는 P1-2c까지 로컬 구현됐고, Production migration과 후보 근거 고도화가 남아 있다.
- 자동 초안·추천 후보 풀·Drag & Drop·지도·실시간 품질 패널은 이미 제품 흐름에 연결되어 있다. 좋은 후보 우선 추천과 동일 좌표 쇼핑 중복 억제도 내부 로직에 반영되어 있다. 이번 갱신으로 자동 초안·일반 후보·실시간 테마 검증이 `activeThemeCategories()`라는 공용 집합을 사용한다.
...
- P1-2a migration `20260818000000_add_project_anchor`는 확인된 로컬 PostgreSQL(`localhost:5432/tour_dna_local`)에만 적용했다. Production Neon에는 migration/apply하지 않았다. 구 스키마 Production에서도 `ProjectAnchor` 테이블이 없으면 분석·실행안 화면은 `UNAVAILABLE` 안내로 계속 열리고, 확정 저장과 Anchor가 포함된 코스 저장만 보류되도록 fallback을 유지한다. 따라서 Production 기준 P1-2 전체는 migration 승인·적용 전까지 “부분 완료”다.
- POI 큐레이션은 운영자 화면 없이 런타임 보조 계층으로만 유지한다. 큐레이션 테이블이 없는 환경에서는 기본 POI 조회로 폴백하며, 이번 문서 갱신에서 Neon DB에는 접근하지 않는다.
...
> 이번 개발 단위에서 **“핵심 테마 슬롯과 후보 재정렬”**을 로컬 코드·36개 지역 A/B·단위 회귀 테스트까지 구현했다. DNA·Network·유사도·전략 점수·Anchor P1-2·축제 API·Neon은 변경하지 않았다. 다음 개발 단위는 **“근거 기반 여행 조건 검증”**이며, 운영자 화면은 제품 범위에 포함하지 않는다.

### 2026-08-18 갱신 — P1-2b 확정 Anchor 코스 고정 연결

#### 이번 단계의 사용자 흐름

- 실행안은 현재 프로젝트의 확정 Anchor를 읽지만 자동으로 코스에 넣지 않는다. 사용자가 `이 축제를 코스에 고정`을 눌렀을 때만 지정된 `plannedDayIndex`에 고정 일정으로 삽입한다.
- 코스에 들어가는 항목은 일반 POI와 구분되는 `kind: FESTIVAL_ANCHOR`이며, `ProjectAnchor.updatedAt`과 행사·날짜·시각 스냅샷을 함께 보존한다. 기존 POI를 삭제하거나 날짜 전체를 다시 설계하지 않는다.
- 코스 반영은 `USER_CONFIRMED`·`CUSTOM`과 정확한 `timeStart`·`timeEnd`가 모두 있을 때만 허용한다. `UNCONFIRMED`·오전/오후/저녁 블록은 가짜 시각을 만들지 않고 정확한 시각 확정을 안내한다.
- Anchor 행은 보라색 고정 일정으로 표시하며 드래그·위/아래·날짜 이동·시간·체류시간 입력을 제공하지 않는다. `코스에서만 제거`는 프로젝트 Anchor 삭제와 별도 동작이다.
- ProjectAnchor가 변경되면 기존 코스 항목은 stale로 표시하고 사용자가 `변경한 Anchor 다시 반영`을 눌러야 snapshot을 교체한다. ProjectAnchor가 삭제되면 기존 코스 항목은 orphan으로 표시하고 명시적 제거를 요구한다.

#### 기존 기능과의 연결

- 일반 POI의 버튼·Drag & Drop 편집 경로는 유지하고 Anchor는 이동 연산에서 차단한다. 일반 POI는 Anchor 주변으로 이동할 수 있으며 Anchor의 확정 시각은 재계산 때 보존된다.
- 지도 마커 InfoWindow와 인쇄 화면에서 Anchor를 별도 라벨로 표시하고, 전략별 POI 적합도 계산에서는 synthetic Anchor ID를 조회하지 않는다.
- 기존 실시간 품질검증을 재사용한다. Anchor의 정확한 체류시간은 이동·시간 충돌 판정에 포함하되, 핵심 테마 비율·일일 POI 과밀 기준에서는 Anchor를 일반 POI로 중복 집계하지 않는다.
- 저장 시 Anchor가 포함된 경우에만 현재 ProjectAnchor를 서버에서 다시 읽어 snapshot과 `updatedAt`을 검증한다. 기존 Anchor가 없거나 저장소가 없는 환경에서는 저장을 거부하며, Anchor가 없는 레거시 코스 저장은 기존 경로를 유지한다.

#### 범위에서 제외한 것

- 축제 전후 관광·식음·숙박 자동 추천, 수혜 범위 분석, 전략 점수 변경, 전체 코스 재생성, 운영자 화면은 P1-2b에 포함하지 않는다. 다음 작업은 P1-2c 전후 연결이다.
- `20260818000000_add_project_anchor` migration은 로컬 DB에만 적용되어 있다. Neon Production에는 migration·데이터 쓰기·대량 정합성 작업을 하지 않았다.
...
### 2026-08-18 갱신 — P1-2c 축제 Anchor 전후 후보 연결

#### 구현 범위

- `src/lib/services/anchorCandidateService.ts`에서 현재 프로젝트의 유효한 Anchor와 현재 코스 snapshot을 대조한 뒤, 지역 POI를 카테고리별로 한 번에 조회한다. 후보별 외부 API 호출이나 N+1 DB 조회는 하지 않는다.
- 역할은 행사 전 관광·체험(`PRE_EVENT`), 식사(`MEAL`), 행사 후 관광·체험·상권(`POST_EVENT`), 숙박(`STAY`)으로 분리한다. 당일치기에는 숙박 역할을 만들지 않는다. 일반 `추천 후보` 풀은 별도 영역으로 유지한다.
- 기존 `computePoiFit`, 테마 적합도, 추천 표시 가능 여부, 동일 시설·좌표 중복 제거를 재사용한다. 정렬은 역할 적합도 → 테마·전략 관련성 → Anchor와의 Haversine 직선거리 → 카테고리 tier → 이름·ID 순이며 역할별 최대 3개, 전체 최대 12개다.
- 후보 카드는 이름·분류·거리·거리 산정 방식·역할 적합도·추천 이유·운영시간 확인 상태를 표시한다. 운영시간이 없으면 행사 후 이용이나 야간 운영을 가정하지 않고 확인 필요로 표시한다.

#### 삽입·안전 정책

- 행사 전 후보는 Anchor 앞에 넣고, 행사 후 후보는 Anchor 뒤에 재계산한다. 식사는 행사 전/후 라디오 선택을 제공한다. 숙박은 일반 `items`가 아닌 날짜별 `lodging` 슬롯에만 넣으며 기존 숙박이 있으면 추가하지 않는다.
- Anchor의 정확한 시작·종료 시각과 snapshot은 삽입 전후 그대로 보존한다. 자동 삽입·전체 코스 재계획·기존 장소 삭제·전략 점수 변경은 하지 않는다. 추가 후 기존 지도·실시간 품질검증 흐름을 다시 사용한다.
- `USER_CONFIRMED`·`CUSTOM`·정확한 시작/종료 시각·좌표·지역·현재 코스 Anchor가 모두 맞지 않으면 후보를 계산하지 않는다. stale/orphan·저장 구조 미적용은 오류 패널 또는 빈 안내로 처리해 실행안 전체를 막지 않는다.
- 반경을 임의로 잘라내지 않는다. 실제 로컬 `FESTIVAL` POI를 Anchor 위치 대용으로 읽은 50개 표본에서 가장 가까운 관광지/음식점 직선거리의 p50/p75/p90은 각각 관광지 `0.12/0.28/0.61km`, 음식점 `0.36/0.77/2.78km`였다. 이 값은 관찰치일 뿐 지역별 고정 반경으로 사용하지 않고, 후보는 거리 순으로만 우선 정렬한다.

#### 검증·데이터 범위

- 대전 전용 QA 프로젝트에서 행사 전 후보를 추가해도 고정 Anchor 시각과 일반 후보 풀이 유지되는지 확인했다.
- 세종 전용 QA 프로젝트에서 행사 전·식사·행사 후·숙박 4개 역할, 직선거리와 운영시간 확인 상태를 확인했다.
- 제천 전용 QA 프로젝트에서는 현재 후보를 모두 코스에 포함한 뒤 빈 연계 후보 상태와 실행안 정상 노출을 확인했다. 기존 제천 사용자 프로젝트는 변경하지 않았다.
- 이번 단계의 QA fixture·조회·데이터 쓰기는 `localhost:5432/tour_dna_local`에서만 수행했다. Neon Production에는 접속·migration·데이터 쓰기를 하지 않았다.
- 단위 테스트에는 후보 역할·거리·중복·stale·미확정·기존 POI 제외와 행사 전/후·숙박 삽입을 추가했고, 실제 Chromium Anchor E2E 3건을 통과시켰다. 기존 Drag & Drop·지도 회귀 E2E도 함께 통과했다.

### 2026-08-18 갱신 — P1-2a 프로젝트 Anchor 저장·확정

#### 저장 모델 결정

- `preferredThemes`, `excludedThemes`, `ProjectInput.memo`, `SelectedPlan.course`에 축제 ID를 섞지 않았다. Anchor는 테마·여행 조건·실행안과 의미가 다른 프로젝트 기획 계기이므로 `ProjectAnchor` 독립 모델을 선택했다.
- 현재 프로젝트당 1건(`projectId @unique`)을 확정하며, 이후 복수 Anchor·이력으로 확장할 때도 기존 입력 JSON 계약을 깨지 않는다. 변경은 같은 프로젝트 행을 upsert하고, 삭제는 Anchor 행만 제거한다.
- 공식 식별자는 `source=TOUR_API_FESTIVAL`, `sourceId=contentid`, `contentTypeId=15`를 보존한다. 행사명·행사 시작/종료일·주소·좌표·프로젝트 지역 코드와 사용자가 확정한 `plannedDate`·`plannedDayIndex`를 별도 필드로 저장한다.
- `sourceSnapshot`에는 후보 카드에 표시한 최소 사실만, `provenance`에는 한국관광공사 dataset·지역·여행월·조회 시각·응답 건수를 저장한다. `rawPayload` 전체는 저장하지 않는다. 저장 후 원천 API가 바뀌어도 기존 확정 Anchor를 자동 덮어쓰지 않는다.

#### 시간·날짜 확정 정책

- 프로젝트에는 실제 여행 시작일 입력이 없으므로 행사 시작일을 여행 시작일로 추정하지 않는다. 사용자가 행사 기간과 여행월의 교집합 날짜 중 하나를 직접 고르고, 여행 일차도 1~여행 기간 범위에서 직접 지정한다.
- 현재 `searchFestival2` 응답에는 정확한 행사 시작·종료 시각이 없으므로 임의 시각을 만들지 않는다. 사용자는 `공식 시각 미확정`을 명시하거나 오전·오후·저녁 기획 블록 또는 직접 입력한 `HH:mm` 시간대를 선택한다. 직접 입력 시간은 공식 행사 시각이 아니라 기획자 지정 조건으로 표시한다.
- 후보 선택만으로는 서버에 저장하지 않는다. 날짜·일차·시간 조건을 모두 명시하고 `Anchor 확정`을 눌러야 서버 저장된다. 저장해도 이번 단계에서는 코스 삽입·재정렬·추천 후보 변경·전략 점수 변경을 하지 않는다.

#### 권한·동시성·하위 호환

- `assertProjectAccessible`와 프로젝트 접근 쿠키를 모든 Anchor Server Action에서 재사용한다. UI를 거치지 않은 직접 POST도 같은 가드를 통과해야 한다.
- 저장·삭제 시 화면이 읽은 `Project.updatedAt`를 조건으로 `Project.updateMany`를 먼저 수행한다. 다른 브라우저가 프로젝트를 변경했으면 Anchor upsert/delete를 하지 않고 새로고침을 요구한다.
- 기존 P1-1의 프로젝트별 `localStorage` 선택은 읽어 오되 자동 업로드하지 않는다. 서버 Anchor가 존재하면 `localStorage`보다 서버 값을 우선하고, 서버 migration이 없는 배포에서는 후보 확인만 허용하며 확정 저장을 실패 상태로 정직하게 안내한다.
- 분석 화면은 `ProjectAnchor` 테이블이 없는 구 Production에서도 전체 500이 되지 않고 저장 구조 미적용 상태를 안내한다. 실행안 페이지도 현재 Anchor를 읽을 수 있는 경로를 미리 연결했지만, 이번 단계에서 코스에 노출하거나 삽입하지 않는다.

#### 이번 단계 검증 범위

- 도메인 테스트: 공식 `contentid`·contentTypeId·스냅샷·provenance, 행사 기간 교집합, 날짜/일차 범위, `UNCONFIRMED`·기획 시간대·직접 입력 시간, invalid 입력을 검증한다.
- 저장 서비스 테스트: 서버 read, migration 미적용 fallback, optimistic concurrency, upsert·delete를 검증한다.
- Server Action 테스트: 프로젝트 접근 가드, 서버 공식 후보 재조회, 명시 조건 검증, 저장·삭제 redirect 경로를 검증한다.
- 화면 테스트: 기존 localStorage 복원, 서버 Anchor 우선, 확정 날짜·일차·시간 표시를 검증한다. 전체 회귀는 최종 검증 결과를 이 문서 하단과 커밋 보고에 반영한다.

### 2026-08-18 갱신 — P1-1 지역·월 기반 축제 Anchor 후보 연계

#### 조사 결과

- 로컬 PostgreSQL을 읽기 전용으로 확인했다. `Region`은 SIDO 16개·SIGUNGU 255개이며, `Poi.category=FESTIVAL`은 906건(226개 SIGUNGU)이다.
- FESTIVAL POI 906건 중 API 원천 903건, fixture 3건이다. 좌표는 906/906건, 주소는 906/906건, `rawPayload`는 903/906건에 있다.
- API 원본 903건은 `TOUR_INFO`의 `areaBasedList2` 응답이며 `contentid`, `title`, `addr1`, `mapx`, `mapy`, `contenttypeid=15`, `lDongRegnCd`, `lDongSignguCd`는 있으나 `eventstartdate`, `eventenddate`는 0/903건이었다. 따라서 기존 POI의 이름·분류·수집 시점으로 여행월 축제를 추정하지 않는다.
- 행사 날짜가 실제로 포함된 공식 원천은 한국관광공사 `searchFestival2`다. 2026년 8월 전국 읽기 조회에서 `eventstartdate`, `eventenddate`, `lDongRegnCd`, `lDongSignguCd`, 좌표·주소·`contentid`를 실제 확인했고, 이 API의 기간 필터와 법정동 코드를 함께 사용한다.

#### 구현 범위

- `src/lib/public-data/adapters/festival.ts`: `searchFestival2`의 실제 행사 날짜·지역·좌표 필드를 Zod로 검증하고, 페이지 응답을 정규화한다.
- `src/lib/domain/festivalAnchor.ts`: `YYYYMMDD` 유효성, 윤년 포함 여행월 범위, 양 끝 날짜를 포함한 기간 겹침, 시도·시군구 코드 일치, `contentid` 중복 제거를 순수 함수로 처리한다.
- `src/lib/services/festivalAnchorService.ts`: 프로젝트 지역·여행 연도·월을 기준으로 공식 API를 읽고 `AVAILABLE`·`EMPTY`·`UNAVAILABLE`·`ERROR`를 구분한다. API 키·지역 코드·DataSource가 없거나 API 오류일 때 후보를 지어내지 않는다.
- `src/components/festival/FestivalAnchorPanel.tsx`: 분석 화면에 후보 카드, 행사 기간·주소·출처·조회 시각, 선택/해제 상태, empty/error 안내를 표시한다. 선택해도 코스에 자동 삽입하지 않고 전략 점수도 바꾸지 않는다.
- 선택 상태는 이번 단계에서 `localStorage`에 프로젝트별로 유지한다. 이는 새로고침 성공 기준을 충족하는 임시 UI 상태이며, P1-2에서 서버 공유·권한·이력까지 필요한 별도 Anchor 저장 구조를 승인 후 결정한다. `preferredThemes`에 축제 ID를 섞지 않았고, Prisma migration·Neon 쓰기·운영자 화면은 추가하지 않았다.

#### 대표 데이터 감사(로컬 DB)

| 지역 | 로컬 FESTIVAL POI | API 원천 | 날짜 필드가 있는 기존 POI |
|---|---:|---:|---:|
| 대전 유성구 | 8 | 8 | 0 |
| 세종특별자치시 | 7 | 7 | 0 |
| 제천시 | 4 | 4 | 0 |
...
위 표는 기존 정적 POI 공급량 감사이고, 월별 실제 후보 공급량은 사용자가 입력한 연도·월에 대해 `searchFestival2`를 조회한 뒤 응답된 행사 기간과 법정동 코드로 다시 판정한다. 따라서 “지역에 FESTIVAL POI가 있다”를 “해당 월에 운영되는 축제가 있다”로 해석하지 않는다.

2026년 8월을 동일 조건으로 공식 `searchFestival2`에 읽기 조회한 대표 표본은 다음과 같다. `matchedCount`는 응답의 법정동 코드까지 다시 확인한 건수이며, 날짜·좌표·주소가 모두 있는 건수도 함께 확인했다.

| 지역 코드 표본 | API 응답 건수 | 지역 일치 건수 | 날짜 완비 | 좌표 완비 | 주소 완비 |
|---|---:|---:|---:|---:|---:|
| 대전 유성구(30/200) | 1 | 1 | 1 | 1 | 1 |
| 세종(36110) | 1 | 1 | 1 | 1 | 1 |
| 제천시(43/150) | 0 | 0 | 0 | 0 | 0 |
| 청주시 흥덕구(43/113) | 0 | 0 | 0 | 0 | 0 |
| 강릉시(51/150) | 2 | 2 | 2 | 2 | 2 |
| 경주시(47/130) | 3 | 3 | 3 | 3 | 3 |
| 춘천시(51/110) | 1 | 1 | 1 | 1 | 1 |
| 여수시(12/130) | 2 | 2 | 2 | 2 | 2 |
| 부산 중구(26/110) | 0 | 0 | 0 | 0 | 0 |

#### 검증

- `npm run typecheck` 통과.
- `npx vitest run tests/unit/festivalAnchor.test.ts tests/unit/FestivalAnchorPanel.test.tsx` 통과(2개 파일, 5 tests).
- 2026년 8월 대표 지역 9곳의 공식 `searchFestival2` 읽기 조회에서 지역 코드·날짜·좌표·주소 공급을 확인했다. 제천·청주 흥덕구·부산 중구처럼 해당 월 결과가 0건인 지역은 empty 상태로 처리된다.
- `npm run lint` 통과.
- `npm test` 통과(111개 파일, 1,557 tests).
...
## 2026-08-17 갱신(2) — Phase C 10번 1차(문화예술 테마 기반)

Phase B 완료 다음 단계인 Phase C를 작은 단위로 시작했다. 이번 단위는 새 전략 템플릿이나 DB schema를
추가하지 않고, 기존 `CULTURE_HISTORY` 전략을 재사용할 수 있는 `CULTURE_ARTS` 세부 테마 기반을
연결하는 범위다.

**1) 자유 입력 분류**: `ThemeCategory`에 `CULTURE_ARTS`를 추가하고 `문화예술`, `문화 예술`, `예술`,
`미술`, `전시`, `공연`, `갤러리`, `아트` 키워드를 별도 분류한다. 기존 `문화`·`역사` 키워드와의
하위 호환은 유지한다.

**2) 공식 구조 신호**: TourAPI `lclsSystm2=VE07`(전시시설)를 기존 `CULTURE_HISTORY`와 새
`CULTURE_ARTS`에 동시에 매핑한다. 박물관·기념관·전시관·과학관·미술관 등이 기존 문화·역사 코스에서
계속 적합 판정을 받으면서 문화예술 선호에서도 구조적으로 우선될 수 있다. `VE` 대분류 전체를
확장하지 않고 검증된 `VE07` 중분류만 사용한다.

**3) 기존 파이프라인 연결**: 문화예술을 `ATTRACTION` 우선순위, 사업기회 라벨, 실시간 코스 품질검증
라벨에 연결했다. 템플릿 가산점은 `CULTURE_HISTORY` 전략 8점으로 제한해 기존 전략의 핵심 테마
판정을 바꾸지 않고 세부 선호만 반영한다. 입력 폼 예시에도 `문화예술`을 추가했다.
...
## 2026-08-17 갱신(6) — Phase C 11번 레저·액티비티 공식 중분류 1차

레저·액티비티를 장소명 키워드로 겨울스포츠·골프·익스트림까지 추정하지 않고, TourAPI 공식 중분류가
확인된 범위만 설명 가능하게 연결했다.

**1) 공식 코드 확인**: `lclsSystmCode2(lclsSystm1=LS)` 실 응답에서 다음 4개를 확인했다.
...
| `LS02` | 수상레저스포츠 | `WATER` |
| `LS03` | 항공레저스포츠 | `AIR` |
| `LS04` | 복합레저스포츠 | `COMBINED` |

**2) 로컬 데이터 감사**: 로컬 DB의 `EXPERIENCE` 3,774건에서 `LS01` 1,035건, `LS02` 468건,
`LS03` 36건, `LS04` 43건을 확인했다. `AC05` 1,986건·`VE10` 198건·`VE12` 2건·코드 없음
5건은 `LS`로 단정하지 않았다. 공식 분류가 넓어 보이는 일부 장소명도 확인됐으므로, 이번 단위에서는
세부 유형을 재추정하거나 권장 체류시간을 하드코딩하지 않는다.

**3) 구현**: `src/lib/domain/leisureClassification.ts`에 `LS01~LS04` 순수 분류기를 추가하고,
추천 후보 카드와 코스 항목의 적합도 근거에 공식 분류를 표시한다. 운영시간·휴무일 advisory가 발생한
레저 항목에도 같은 분류를 함께 표시한다. 기존 `LEISURE_ACTIVITY` 테마 판정과 `EXPERIENCE`
카테고리 매핑은 유지하며 DB schema·저장 구조는 변경하지 않았다.
...
이번 작업은 문화예술·레저 상세 운영정보를 별도 기능으로 계속 쪼개지 않고, 전략·검증·증분 반영을
하나의 운영 단위로 묶어 마무리했다.

**1) 문화예술 전용 전략 템플릿**: `CULTURE_ARTS`를 `STRATEGY_TEMPLATES`에 추가했다. 전시·공연·
문화예술공간과 인근 식음·상권 연계를 전략 concept·KPI·위험요인·협력기관·예산 설명에 반영한다.
`CULTURE_ARTS` 테마의 전용 가산점과 핵심 테마 판정도 연결했으며 기존 `CULTURE_HISTORY` 전략은
하위 호환으로 유지한다. 전략 점수 공식·DB schema·분석 저장 구조는 변경하지 않았다.

**2) 복합 운영시간 보수 판정**: 단일 `HH:MM~HH:MM`만 일정 범위와 비교한다. 요일·시즌·공휴일·
회차·입장마감 조건 또는 복수 시간대가 포함되면 자동 영업 가능 판정을 생략하고 확인 advisory를
표시해, 예를 들어 평일/주말 시간이 다른 시설을 한쪽 시간대에 맞았다는 이유로 통과시키지 않는다.

**3) 전국 제한형 증분 CLI**: 기존 `--region-code=SGG_... --max-items=N`에 더해
`--all-regions --max-items=N`을 지원한다. SIGUNGU 전체를 코드순으로 조회하지만 상세 API 호출 총량은
실행당 1~100건으로 제한하며, 후보 조건·localhost DB 가드·순차 호출·기존 raw payload 보존 정책은
동일하다. 전국 백필 자체는 호출량·실패 재처리 정책을 확인하며 여러 회차로 실행한다.

**4) 검증**: `typecheck` 통과, 관련 단위 테스트 4개 파일 70개 통과. 전국 옵션은 CLI 파싱 테스트와
로컬 DB 1건 실행으로 확인한다. 전체 테스트·lint·production build는 커밋 전 최종 검증에서 다시
실행한다.

## 2026-08-19 갱신 — 여행 조건 공공데이터 가용성 조사

이번 단위는 기능 구현이 아닌 조사·판정 문서화다. 한국관광공사 공식 명세와 현재 로컬 PostgreSQL을 읽기 전용으로 대조했다. Neon 접근·migration·대량 동기화·POI 추천/필터 변경은 하지 않았다.

### 1. Git 상태

- 브랜치: `main`
- 조사 시작 기준: `HEAD=origin/main=43d9c5b8721e94bbd1f0400468e89d395f32ccba`
- 보호 대상 `scripts/delete-route-geometry-verification-project-3.mts`, `tour_dna_local.dump`는 건드리거나 stage하지 않는다.

### 2. 현재 여행 조건 구조

- 입력 코드는 `CONDITION_PET_FRIENDLY`, `CONDITION_ACCESSIBLE`, `CONDITION_WALK_TRANSIT`, `CONDITION_FAMILY` 4개다.
- `ProjectInput.preferredThemes` JSON에 `{ version: 1, themes, travelConditions }`로 저장하며 별도 여행 조건 컬럼은 없다.
- `Poi`는 `externalId`에 API `contentid`를 보관하고, `contenttypeid`는 `rawPayload`에 보관한다. 무장애·반려동물 전용 컬럼·adapter·정규화 모델은 없다.
- 화면은 두 조건을 선택·표시하고 “실제 시설 정보 확인 전까지 확정하지 않는다”고 안내할 뿐, 분석·후보·검증의 사실 판정에는 연결하지 않는다.

### 3. 무장애 공식 API/endpoint

공식 데이터셋은 [한국관광공사_무장애 여행 정보](https://www.data.go.kr/data/15101897/openapi.do)이며 base URL은 `https://apis.data.go.kr/B551011/KorWithService2`다. 핵심 endpoint는 `areaBasedList2`, `areaBasedSyncList2`, `detailWithTour2`이고, 공통·소개·반복 상세도 같은 서비스에 있다. 공식 페이지는 실시간 갱신, 무료, 개발계정 1,000회 트래픽으로 표시한다. 서비스 URL 변경 사항은 [한국관광공사 API 변경 공지](https://www.data.go.kr/bbs/ntc/selectNotice.do?originId=NOTICE_0000000004082)에 기록돼 있다.

### 4. 무장애 실제 필드

공식 `detailWithTour2` 응답은 `contentid`와 다음 28개 문자열 필드를 제공한다: `wheelchair`, `exit`, `elevator`, `restroom`, `guidesystem`, `blindhandicapetc`, `signguide`, `videoguide`, `hearingroom`, `hearinghandicapetc`, `stroller`, `lactationroom`, `babysparechair`, `infantsfamilyetc`, `auditorium`, `room`, `handicapetc`, `braileblock`, `helpdog`, `guidehuman`, `audioguide`, `bigprint`, `brailepromotion`, `parking`, `route`, `publictransport`, `ticketoffice`, `promotion`.

### 5. 무장애 필드 판정 가능 수준

- 원천·endpoint·필드 계약: 확인됨.
- 현재 키의 실제 접근: `KorWithService2/areaBasedList2` 최소 호출이 HTTP 403으로 거부됨. 서비스 활용신청/권한 확인이 먼저다.
- 현재 DB 원문: 위 28개 필드 또는 `detailWithTour2` 구조를 가진 `Poi` 0/48,291건.
- 결론: 데이터 소스 조사는 완료됐지만, 지금은 무장애 가능/불가 필터나 점수를 안전하게 만들 수 없다. 구현 시 `AVAILABLE`과 `UNKNOWN`을 분리해야 하며 빈 문자열을 불가로 해석하면 안 된다.

### 6. 반려동물 공식 API/endpoint

공식 데이터셋은 [한국관광공사_반려동물_동반여행_서비스](https://www.data.go.kr/data/15135102/openapi.do)이며 base URL은 `https://apis.data.go.kr/B551011/KorPetTourService2`다. 핵심 endpoint는 `areaBasedList2`, `petTourSyncList2`, `detailPetTour2`다. 일반 관광 서비스 `https://apis.data.go.kr/B551011/KorService2`에도 `detailPetTour2`가 제공되며, 2026년 URL 변경은 [반려동물 API 변경 공지](https://www.data.go.kr/bbs/ntc/selectNotice.do?originId=NOTICE_0000000004471)에 기록돼 있다.

### 7. 반려동물 실제 필드

공식 `detailPetTour2` 응답은 `contentid`, `relaAcdntRiskMtr`, `acmpyTypeCd`, `relaPosesFclty`, `relaFrnshPrdlst`, `etcAcmpyInfo`, `relaPurcPrdlst`, `acmpyPsblCpam`, `relaRntlPrdlst`, `acmpyNeedMtr`다. 실제 읽기 전용 호출에서 `contentId=2930927`은 `acmpyTypeCd=전구역 동반가능`을 반환했고 나머지 항목은 빈 문자열이었다. 즉 단순 boolean이 아니라 동반 범위·필요사항·시설·제한을 함께 다루는 원문 구조다.

### 8. 반려동물 판정 가능 수준

- 원천·endpoint·필드 계약: 확인됨.
- 현재 키의 실제 접근: `areaBasedList2`는 HTTP 200·`resultCode=0000`, 전체 9,693건을 반환했고 `petTourSyncList2`는 10,153건을 반환했다. `detailPetTour2`도 HTTP 200으로 확인했다.
- 현재 DB 원문: 위 반려동물 상세 필드를 가진 `Poi` 0/48,291건.
- 결론: 무장애보다 API 접근 검증은 한 단계 앞섰지만, 현재 제품에서 반려동물 조건을 확정 필터로 쓰기에는 저장·정규화·최신성 관리가 없다.

### 9. 현재 POI와 contentId 연결률

- 감사 모집단은 추천 로직을 재실행하지 않은 전체 `Poi` 48,291건이다.
- `Poi.externalId` 비공백: 48,291/48,291, 100.00%; 중복 값 0건.
- `rawPayload.contentid` 비공백: 48,268/48,291, 99.95%.
- `rawPayload.contenttypeid` 비공백: 48,268/48,291, 99.95%.
- 조건 상세 endpoint는 `contentId`만으로 조회할 수 있으므로, ID 기준 조건 상세 호출 상한은 조건별 48,291건이다. `contentTypeId` 누락 23건은 일반 소개 상세와 분류 결합 시 별도 보완 대상이다.

### 10. category별 coverage 비교

| category | POI | externalId | contentTypeId | 무장애 원문 | 반려동물 원문 |
|---|---:|---:|---:|---:|---:|
| `ATTRACTION` | 15,146 | 15,146 | 15,140 | 0 | 0 |
| `FOOD` | 13,364 | 13,364 | 13,359 | 0 | 0 |
| `LODGING` | 2,953 | 2,953 | 2,950 | 0 | 0 |
| `SHOPPING` | 12,148 | 12,148 | 12,147 | 0 | 0 |
| `FESTIVAL` | 906 | 906 | 903 | 0 | 0 |
| `EXPERIENCE` | 3,774 | 3,774 | 3,769 | 0 | 0 |

### 11. 전국 SIGUNGU coverage

- `Region.level=SIGUNGU`는 255개이며, 255/255 지역에 POI가 있다.
- 255/255 지역에 `externalId`가 하나 이상 있고, 255/255 지역에서 모든 POI가 `externalId`를 가진다.
- 255/255 지역에 `contentTypeId`가 하나 이상 있고, 252/255 지역은 모든 POI가 `contentTypeId`를 가진다.
- 지역별 POI 수는 최소 7, 중앙값 141, 최대 1,269건이다. 지역별 `externalId` 연결률은 최소·중앙값·최대가 모두 100%이며, `contentTypeId` 연결률은 최소 96.76%, 중앙값 100%, 최대 100%다.
- 무장애·반려동물 상세 원문은 255개 지역 모두 0건이다. “지역별 데이터가 이미 있다”와 “조건 근거가 있다”는 같은 의미가 아니다.

### 12. 경주·강릉·제천·청주·대전·세종 결과

`대전`은 저장된 대표 코드가 `SGG_DAEJEON`이며 명칭은 유성구다.

| 지역 | POI | externalId | contentTypeId | 무장애 원문 | 반려동물 원문 |
|---|---:|---:|---:|---:|---:|
| 경주시 | 620 | 620 | 620 | 0 | 0 |
| 강릉시 | 1,001 | 1,001 | 1,001 | 0 | 0 |
| 제천시 | 253 | 253 | 246 | 0 | 0 |
| 청주시 흥덕구 | 141 | 141 | 141 | 0 | 0 |
| 대전 유성구 대표 | 396 | 396 | 388 | 0 | 0 |
| 세종특별자치시 | 204 | 204 | 204 | 0 | 0 |

### 13. missing/negative 처리 원칙

- 빈 문자열, 상세 미호출, 권한 오류, timeout, `NO_DATA`는 모두 `UNKNOWN` 또는 `UNAVAILABLE` 계열로 보존한다.
- 공식 원문에 명시된 제한·불가 문구만 `RESTRICTED`/`NOT_AVAILABLE` 후보로 분류한다. 빈 값에서 불가를 추론하지 않는다.
- 시설명·카테고리·키워드만으로 무장애·반려동물 가능을 추정하지 않는다. 사업자 확인이나 공식 원문이 없으면 사용자에게 확정 문구를 보여주지 않는다.
- `NO_DATA`와 “공식 원문에 불가가 적힘”은 별도 상태로 저장해야 한다.

### 14. 데이터 최신성

- 로컬 `TOUR_INFO` 최신 스냅샷: `baseYm=202606`, `fetchedAt=2026-08-11T04:21:32.888Z`.
- 전국 POI 동기화 최신 성공 로그 종료: `2026-08-11T04:21:32.900Z`.
- 현재 `Poi` 48,291건의 `rawPayload`에는 원천 `modifiedtime`이 보존되지 않아 POI별 원천 최신성을 판정할 수 없다.
- 운영시간 상세가 저장된 3건은 2026-08-16에 별도 조회됐지만, 이는 여행 조건 원문 동기화가 아니다.
- 공식 데이터셋 페이지의 수정일은 두 조건 서비스 모두 2026-02-26, 업데이트 주기는 실시간으로 표시된다. 따라서 조건 원문은 `fetchedAt`·원천 수정시각·상태를 함께 보존해야 한다.

### 15. 전국 enrichment 예상 API 호출량

- 현재 local POI 전체를 두 조건으로 상세 보강하면 조건별 최대 48,291회, 두 조건 합계 96,582회다. 개발계정 1,000회 한도를 고려하면 한 번에 전국 처리할 수 없다.
- 반려동물은 목록 응답 기준 9,693건(`areaBasedList2`), 동기화 목록 기준 10,153건(`petTourSyncList2`)이다. 목록을 먼저 받아 현재 POI와 교집합을 만든 뒤 상세 호출을 줄여야 한다.
- 무장애는 공식 페이지가 약 6만 건 규모로 설명하지만, 현재 키가 403이어서 실시간 전국 건수는 확정하지 않았다. 페이지 크기 100을 가정해도 목록만 약 600회이며, 상세 호출은 교집합 수만큼 추가된다.
- 따라서 전국 “상세 전수 호출”은 다음 작업으로 바로 실행하지 않는다. 지역·조건별 증분 큐와 일일 상한이 먼저다.

### 16. 저장/cache 구조 권장안

- 현 `Poi.rawPayload`에 조건 원문을 계속 섞기보다 조건별 상세 레코드를 별도 저장하는 방향을 권장한다. 최소 키는 `poiId`, `conditionType`, `contentId`, `source`, `rawPayload`, `fetchedAt`, 원천 `modifiedtime`, `status`다.
- `status`는 `NOT_FETCHED`, `AVAILABLE`, `RESTRICTED`, `NO_DATA`, `ERROR`, `UNAVAILABLE`처럼 API 상태와 원문 판정을 분리한다.
- `conditionType+contentId` 유일 키, 원문 보존, 재시도 횟수·마지막 오류·다음 재시도 시각, 원천 변경 시각을 둔다. 이는 설계 권장안이며 이번 단위에서는 schema/migration을 만들지 않았다.

### 17. 후보 filter/ranking/advisory 권장 수준

- 초기 filter는 `AVAILABLE` 또는 공식 원문에 명시된 허용 정보가 있는 경우만 통과시키고, `UNKNOWN`은 삭제하지 말고 “확인 필요” 후보로 분리한다.
- ranking은 조건 적합성을 단독 강제점수로 크게 주지 말고, 원문 완전성·최신성·제한 존재 여부를 보조 신호로 사용한다. 상세 원문이 없는 POI를 이름이나 테마만으로 상향하지 않는다.
- `RESTRICTED`는 조건 후보에서 제외하거나 별도 경고 영역에 둔다. API 오류·권한 부족은 `RESTRICTED`가 아니라 `UNAVAILABLE`이다.
- 조건별 근거가 충분히 쌓인 뒤에만 후보 풀·실시간 검증에 연결하며, 이번 조사에서는 ranking/filter를 변경하지 않았다.

### 18. 사용자에게 안전하게 표시할 문구

- 근거 있음: “한국관광공사 공식 원문에서 조건 정보를 확인했습니다. 방문 전 운영자에게 최종 확인하세요.”
- 일부 근거: “조건 관련 공식 정보가 일부 확인됩니다. 세부 제한·이용 범위는 원문과 시설에 재확인하세요.”
- 미확인: “이 POI의 무장애/반려동물 동반 가능 여부는 공식 원문에서 확인되지 않았습니다. 추천 확정 전 직접 확인이 필요합니다.”
- API 불가: “현재 공식 조건 정보를 불러오지 못했습니다. 가능하다고 단정하지 않고 확인 필요로 표시합니다.”
- 불가/제한 명시: “공식 원문에 제한 또는 불가 내용이 있어 조건 후보에서 우선 제외했습니다. 변경 여부는 시설에 확인하세요.”

### 19. 무장애 최종 데이터 준비도

**조사 완료·사용자 기능 미준비.** 공식 서비스·endpoint·28개 필드·전국 데이터 규모는 확인됐지만 현재 API 키가 403이고, 로컬 저장 원문은 0/48,291건이다. 다음 구현은 활용신청/권한 확인 → 목록 교집합 → 상세 적재 → 원문 판정 순서여야 한다.
...
1. **조건 상세 저장 계약과 상태 모델 확정**: 현재 `Poi`와 JSON만으로는 원문·오류·최신성을 안전하게 구분할 수 없다.
2. **반려동물 소량 증분 adapter/cache 검증**: 현재 키로 정상 호출되고 필드 실응답까지 확인되어 가장 작은 안전 단위다.
3. **무장애 API 활용신청·권한 확인 후 동일한 증분 경로 검증**: 403 상태에서 전국 호출을 시도하면 quota만 소모되고 판정 근거가 생기지 않는다.
4. **두 조건의 `UNKNOWN` 중심 표시와 실시간 검증 연결**: 데이터가 없는 POI를 자동 삭제하거나 불가로 오판하지 않기 위해 마지막에 연결한다.

### 23. 변경 파일/문서

- 이번 조사에서 기능 코드·Prisma schema·migration·환경변수·README는 변경하지 않았다.
- 문서 갱신 대상은 `docs/implementation-status.md`, `docs/implementation-plan.md`다.
- 보호 대상 파일은 stage하지 않으며, 별도 임시 감사 스크립트도 저장소에 남기지 않는다.

### 24. 검증 결과

- 로컬 DB host는 `localhost`로 확인했고 Neon host는 사용하지 않았다.
- 로컬 PostgreSQL 읽기 전용 집계: 48,291 POI, 255 SIGUNGU, 대표 6개 지역, category별 coverage 완료.
- 공식 API 읽기 전용 smoke check: 일반 `KorService2` 목록 응답, 반려동물 목록·동기화·상세 응답, 무장애 권한 거부 상태를 확인했다. 응답 원문·서비스키는 저장하지 않았다.
- 코드 변경이 없으므로 여행 조건 기능 회귀 테스트는 추가하지 않는다. 문서 반영 후 `git diff --check`, `npm run lint`, `npm run typecheck`를 실행한다.

### 25. commit/push/Git 상태
...
## 2026-08-25 갱신 — PET 유지보수 전환·무장애 승인 확인

### PET 최종 상태

반려동물 기능은 다음 범위까지 완료됐다.

- 공식 PET 목록과 local POI 교집합을 계산하는 pipeline
- `modifiedtime`·`showflag` 기반 증분 수집과 최신 evidence cache
- `CONFIRMED`·`CONDITIONAL`·`UNKNOWN` 공식 근거 표시
- 일반 추천 후보·저장 코스·숙박·Anchor 연계 후보의 PET badge와 `INFO` advisory
- 6개 대표 지역 coverage overlap 및 coverage ceiling 재검증

최근 재감사 결과는 다음과 같다.

| 대상 | 전체 | Evidence 있음 | `LISTED_NOT_ENRICHED` | 공식 목록 외 | 현재 coverage | coverage ceiling |
|---|---:|---:|---:|---:|---:|---:|
| 일반 추천 후보 | 46 | 3 | 0 | 43 | 6.5% | 6.5% |
| 저장 코스 POI·숙박 | 99 | 5 | 0 | 94 | 5.1% | 5.1% |
| Anchor 연계 후보 | 24 | 1 | 0 | 23 | 4.2% | 4.2% |

후보·코스 합산 category에서 `ATTRACTION`은 `6/42`, `FOOD`는 `0/69`, `LODGING`은 `0/16`,
`EXPERIENCE`는 `0/4`, `SHOPPING`은 `2/9`였다. `FOOD`·`LODGING`·`EXPERIENCE`의 공식 PET
evidence overlap은 0이다.

공식 목록에 포함되지만 상세가 없는 대상은 현재 후보·코스·Anchor 연계 후보에서 0건이다. 따라서
추가 상세 API 수집만으로 현재 추천 후보 coverage를 높일 수 없다. 공식 목록에 없는 POI는 반려동물
동반 불가가 아니라 계속 `UNKNOWN`이며, 목록 부재를 negative 판정으로 사용하지 않는다.

이에 따라 PET soft ranking은 **보류**하고 hard filter도 도입하지 않는다. PET 기능은 적극 개발
단계에서 **유지보수 단계**로 전환한다. 새로운 candidate/course에 공식 PET 목록 대상이 실제로
등장할 때만 해당 대상에 한정한 targeted enrichment를 실행한다. 공식 목록 전체 전수 상세 수집이나
PET 기반 추천 순서·자동 코스·전략 점수 변경은 진행하지 않는다.

### 무장애 API 승인 상태

사용자가 공공데이터포털에서 한국관광공사 무장애 여행정보 API 활용신청 승인을 직접 확인했다.
승인 상세기능 목록에는 다음 13개가 표시된다.

`/areaCode2`, `/categoryCode2`, `/areaBasedList2`, `/locationBasedList2`, `/searchKeyword2`,
`/detailCommon2`, `/detailIntro2`, `/detailInfo2`, `/detailImage2`, `/detailWithTour2`,
`/areaBasedSyncList2`, `/ldongCode2`, `/lclsSystmCode2`

각 상세기능의 현재 표시 일일 트래픽은 1,000건이다. 이전에 HTTP 403이 발생했던
`/detailWithTour2`와 `/areaBasedSyncList2`가 승인 목록에 포함된 것은 확인했지만, 실제 승인 반영
후 HTTP 200 응답과 응답 필드 정상성은 아직 재검증하지 않았다. 따라서 “활용신청 승인됨”은 확인
사실로 기록하고, “기존 403 해결됨”은 아직 미확인으로 남긴다.

이번 문서 갱신에서는 무장애 API를 재호출하지 않았다. 다음 개발 작업에서 승인된 endpoint 중
필요한 최소 호출만 사용해 HTTP 200·목록 교집합·`detailWithTour2` 상세 필드·cache를 검증한 뒤
`ACCESSIBILITY` evidence pipeline의 구현 여부를 판단한다.

## 2026-08-25 추가 갱신 — 무장애 API 실호출 검증 결과

활용신청 승인 이후 제품 기능을 변경하지 않고, 로컬 PostgreSQL의 Region master와 기존
`fetchPublicDataJson`·`withRequestCounter`만 사용해 읽기 전용으로 확인했다. 목록은 경주·강릉·대전
유성구를 각각 1회 호출했고, 상세는 경주 목록과 local POI가 정확히 겹친 대상 중 category 균형 표본
8건만 호출했다. 목록·상세 원문은 저장하지 않았다.

### Endpoint·지역별 목록 결과

| 지역 | Region master API 코드 | HTTP/resultCode | 공식 목록 전체 | 이번 조회 | local API POI | 교집합 | 교집합률 |
|---|---|---|---:|---:|---:|---:|---:|
| 경주시 | `47/130` | `200 / 0000 OK` | 140 | 100(1페이지) | 620 | 88 | 14.2% |
| 강릉시 | `51/150` | `200 / 0000 OK` | 803 | 100(1페이지) | 1,001 | 91 | 9.1% |
| 대전 유성구 | `30/200` | `200 / 0000 OK` | 79 | 79(전체) | 388 | 69 | 17.8% |

경주와 강릉은 `totalCount` 전체가 각각 140건·803건이지만 호출량을 제한하기 위해 첫 100건만
조회했다. 따라서 두 지역의 교집합률은 전체 공식 목록 기준이 아니라 **조회한 첫 페이지 기준의
부분 측정값**이며, 전체 목록 overlap으로 확대 해석하지 않는다. 목록 endpoint는
`/B551011/KorWithService2/areaBasedSyncList2`이다.

목록 응답은 세 지역 모두 HTTP 200, `resultCode=0000`, `resultMsg=OK`였다. 경주 첫 페이지는
`contentid`, `contenttypeid`, `title`, `addr1`, 법정동 코드, 좌표, `modifiedtime`, `showflag`가
100/100 비어 있지 않았다. `addr2`, 이미지, `areacode`·`cat1`~`cat3`·`sigungucode`는 빈 문자열이
섞였고 `tel`은 표본 전체가 빈 문자열이었다. 빈 문자열은 동반 불가가 아니라 미확인으로 다룬다.

### 목록 교집합 category 분포

| 지역 | `ATTRACTION` | `FOOD` | `LODGING` | `EXPERIENCE` | `SHOPPING` | 합계 |
|---|---:|---:|---:|---:|---:|---:|
| 경주시 | 58 | 9 | 15 | 2 | 4 | 88 |
| 강릉시 | 37 | 32 | 5 | 7 | 10 | 91 |
| 대전 유성구 | 32 | 25 | 7 | 3 | 2 | 69 |

### 상세 응답 필드와 값 패턴

경주 교집합에서 `ATTRACTION 5`, `FOOD 1`, `LODGING 1`, `EXPERIENCE 1`로 8건을 선정했고,
`/B551011/KorWithService2/detailWithTour2`를 8회 호출했다. 8회 모두 HTTP 200,
`0000/OK`, item 1건이었다. 실제 응답 필드는 다음 29개였다.

`audioguide`, `auditorium`, `babysparechair`, `bigprint`, `blindhandicapetc`, `braileblock`,
`brailepromotion`, `contentid`, `elevator`, `exit`, `guidehuman`, `guidesystem`, `handicapetc`,
`hearinghandicapetc`, `hearingroom`, `helpdog`, `infantsfamilyetc`, `lactationroom`, `parking`,
`promotion`, `publictransport`, `restroom`, `room`, `route`, `signguide`, `stroller`, `ticketoffice`,
`videoguide`, `wheelchair`

8건 기준 비어 있지 않은 값은 `parking 7`, `restroom 7`, `route 6`, `exit 4`, `handicapetc 3`,
`wheelchair 2`, `elevator 2`였고, `audioguide`, `bigprint`, 시청각·점자·안내 관련 다수 필드는
8건 모두 빈 문자열이었다. `restroom`에서는 명시적 가능 6건·명시적 불가 1건·빈 값 1건,
`parking`에서는 명시적 가능 7건·빈 값 1건이 관찰됐다. 표본에는 조건부 문구가 없었고,
`handicapetc` 1건은 자유 서술로 분류됐다. 이 분류는 한국어 문구 기반의 검증용 휴리스틱이며,
제품 판정을 위한 정규화 규칙으로 사용하지 않았다.

따라서 현재 API만으로도 차원별 `available`·`unavailable`·`conditional/restricted`·`unknown`
구분을 설계할 수 있다. 다만 빈 문자열이 많은 차원은 `unavailable`로 바꾸면 안 되며, 실제 제품
계약은 전체 상태 하나가 아니라 `wheelchair`, `restroom`, `parking`, `route` 등 차원별 JSON
근거를 보존해야 한다.

### 현재 추천 후보와 `PoiConditionEvidence` 재사용 판단

현재 대표 경주 프로젝트의 일반 추천 후보는 10건이며, 이번 조회 페이지 기준 공식 목록 교집합은
2건(20%), 목록 밖은 8건이다. 이 결과도 공식 목록 140건 전체가 아닌 첫 100건과의 비교이므로
예비 측정이다. Anchor 연계 후보는 해당 프로젝트에서 측정 가능한 후보가 없어 `NOT_AVAILABLE`로
남겼고, 축제 Anchor 자체는 후보 overlap 대상에서 제외했다.

기존 `PoiConditionEvidence`의 `conditionType=ACCESSIBILITY`와 JSON 원문/차원 detail을 담는
근거 envelope는 재사용할 수 있다(**판정 B**). 그러나 PET의 단일 overall 상태로 축약하지 않고,
접근성 차원별 원문·정규화 상태·source modified time을 별도 보존해야 한다. 이번 작업에서는
adapter·schema·migration·evidence 저장을 구현하지 않았고, 기존 접근성 evidence cache도 없으므로
cache hit는 0건이다. 외부 API 재호출 방지는 이번 검증 스크립트가 저장하지 않는 구조상 적용 대상이
없었으며, 동일 실행 안에서 목록과 상세를 무작정 반복 호출하지 않았다.

### 호출·최종 판정

실제 외부 API 호출은 목록 3회와 상세 8회, 합계 **11회**다. 모두 로컬 DB 읽기와 API 읽기뿐이며,
Production Neon 접근·쓰기·migration은 없었다. 무장애 API 준비도는 **PARTIAL**이다. 승인 반영과
필드 구조는 정상이고 경주 후보의 예비 overlap도 확인됐지만, 목록 첫 페이지 측정이 포함되고
상세 표본이 한 지역에만 있어 즉시 제품 pipeline을 구현할 단계는 아니다. 다음은 강릉 또는 대전

...
## Phase 7. 비교 코호트와 행정 범위 설명 — `DONE(0/100 절대값 오해 방지, 2026-08-07)`

- 대전 라벨("대전광역시 (DNA 지표는 유성구 기준)")은 이미 반영됨(직전 세션, `regionQueries.ts`).
- **DNA 점수 상대점수 안내 보완(2026-08-07)**: 화천군 사례("수요 0, 다양성 100" 등이 절대값처럼
  보이는 문제)를 조사한 결과, DNA 산식·정규화 공식 자체는 정상이었고 표현 방식만 개선이 필요했다.
  분석 화면(`/projects/[id]/analysis`)에 다음을 반영했다 — DNA 산식·최소-최대 정규화 공식·가중치는
  전혀 바꾸지 않았다.
  - 점수를 처음 보는 상단 요약 영역과 DNA 상세 영역 두 곳에 "절대평가가 아닌 비교지역 내 상대
    수준"이라는 안내를 hover 없이 기본 노출한다.
  - 0점과 100점 모두 대칭적으로 안내한다. 다만 반올림 때문에 0/100으로 보일 뿐 실제로는 비교지역
    중 정확한 최저·최고가 아닌 경우(화천군의 수요·소비 축이 실제 사례)를 정직하게 구분해
    "비교지역 내 최저/최고"(확정)와 "비교지역 내 매우 낮음/높음"(근접)으로 다르게 표시한다
    (`src/lib/domain/axisScoreInterpretation.ts`, 이미 저장된 Evidence의 정규화값만 다시 읽는
    순수 함수 — 새 DB 필드 없음).
  - 유사지역 비교·관광사업 기회·전략 3안 비교·사업 사전검증 리포트 배지에서 내부 규칙 버전
    문자열(`region-similarity-rules-v1` 등)을 화면에서 제거했다.
  - 일반 점수(0/100이 아닌 값)에도 "상대점수"라는 짧은 라벨을 추가했다.

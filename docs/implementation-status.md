...
- 시나리오 B는 기존 로컬 QA 프로젝트 `[QA E2E] 대전 축제 Anchor`와 `[QA PET 사용자] 경주`를
  사용했다. 대전 Anchor를 코스에 고정한 뒤 저장·재로드했을 때 행사 전·식사·행사 후·숙박 후보
  12개가 표시됐고, 행사 전 후보를 추가했다가 삭제해 원상복구했다. PET·ACCESSIBILITY 조건은
...
- P2: 분석·실행안의 정보량, 조건 advisory badge 반복, 홈의 최근 프로젝트 목록 밀도는 여전히
  ...
  다시 설계하지 않았다. 대전 QA fixture의 행사명이 `2025 유성온천 크리스마스 축제`인데 행사일이
  `2026-09-10~2026-09-12`로 표시되는 데이터 불일치는 제품 코드가 아닌 데모 fixture 정합성 위험으로
  기록한다.
...
이번 라운드는 P0/P1 코드 수정 없이 QA 문서만 갱신했다. `npm run test:e2e:local`은 33건 중
`31 passed / 2 skipped`였고, skip 2건은 stale/orphan Anchor fixture가 없는 기존 선택적 skip이다.
정확한 `390×844` 모바일·overflow·접기/펼치기·badge 회귀는 `animated-details-mobile.spec.ts`에서
통과했다. 인앱 브라우저의 고정 뷰포트 때문에 수동 브라우저에서 390px를 직접 설정하는 검증은
수행하지 않고 해당 자동 회귀 결과로 대체했다.

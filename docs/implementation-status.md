# 구현 상태 (2026-08-18 갱신 — P1-1 축제·이벤트 Anchor 후보 연계 반영)
...
| 5. 특별한 기획 계기 | 부분 완료(P1-1) | 지역·여행월 기준 공식 행사정보를 조회하고 기간이 겹치는 Anchor 후보를 선택할 수 있음 | 선택한 축제를 서버 저장·코스 고정 일정·전후 관광/식음/숙박 연결로 확장 |
...
- 9단계 기준으로 완전 완료 5개(1·2·6·7·9), 대부분 완료 1개(3), 부분 완료 3개(4·5·8)다. 5단계는 P1-1(후보 탐색·선택)까지 진행됐다.
...
- 축제 연계는 P1-1에서 한국관광공사 `searchFestival2`의 행사 시작일·종료일·법정동 코드를 사용해 지역·월 후보를 표시하고 선택할 수 있다. 기존 `areaBasedList2`의 FESTIVAL POI에는 날짜가 없어서 날짜를 추정하지 않는다. 선택 상태는 이번 단계에서 브라우저 저장소에만 유지하며, 다음 단계에서 별도 프로젝트 Anchor 저장 구조와 코스 연결을 검토한다.
- POI 큐레이션은 운영자 화면 없이 런타임 보조 계층으로만 유지한다. 큐레이션 테이블이 없는 환경에서는 기본 POI 조회로 폴백하며, 이번 문서 갱신에서 Neon DB에는 접근하지 않는다.

### 다음 작업 순서

1. **P1-2 축제 Anchor 저장·코스 연결**: 선택 상태를 프로젝트 단위로 안전하게 저장하고, 사용자가 명시적으로 확정한 경우에만 축제를 고정 일정으로 넣어 전후 관광·식음·숙박 후보를 연결한다. 자동 삽입·전략 점수 변경은 하지 않는다.
...
> 이번 개발 단위에서 **“지역·여행월 기반 축제·이벤트 Anchor 후보 조회·선택(P1-1)”**을 완료했다. 다음 개발 단위는 **“선택 Anchor의 프로젝트 저장과 코스 연결(P1-2)”**이다. 운영자 화면 구축이나 Neon DB 마이그레이션은 이번 작업에 포함하지 않았다.

...
| 2. 해결하고 싶은 관광 문제 | 대부분 완료 | 체류·소비 확대, 방문객 성장, 재방문, 계절성 균형, 지역경제 등 목표 선택과 DNA 연결 | 체류 확대와 소비 확대를 별도 목표로 분리하고 관광객 분산을 명시적 선택지로 제공 |
| 3. 콘텐츠 테마 | 부분 완료 | 자유 입력 테마를 음식·자연·웰니스·문화역사·문화예술·레저·축제 등으로 분류 | 8개 핵심 테마(미식 / 자연 / 웰니스 / 문화·역사 / 문화예술 / 레저·액티비티 / K-콘텐츠 / 야간관광)를 고정 구조로 제공 |
| 4. 여행 조건 | 부분 완료 | 가족·뚜벅이/대중교통 등 일부 조건이 입력·추천 로직에 반영 | 반려동물·무장애를 테마와 분리된 구조화 조건으로 추가하고 POI 근거와 연결 |
| 5. 특별한 기획 계기 | 부분 완료 | 축제 키워드·축제 기획자 역할·축제 연계 전략 템플릿 존재 | 지역·월 기반 실제 이벤트 탐색, 축제를 고정 일정(Anchor)으로 연결하는 흐름 필요 |
...
- 9단계 기준으로 완전 완료 4개(1·6·7·9), 대부분 완료 1개(2), 부분 완료 4개(3·4·5·8)다.
- 자동 초안·추천 후보 풀·Drag & Drop·지도·실시간 품질 패널은 이미 제품 흐름에 연결되어 있다. 좋은 후보 우선 추천과 동일 좌표 쇼핑 중복 억제도 내부 로직에 반영되어 있다.
- 아직 제품의 출발점이 자유 텍스트 중심이다. 따라서 테마와 여행 조건을 구조화하지 않으면 K-콘텐츠·야간관광·무장애·반려동물 조합을 안정적으로 추천하거나 검증하기 어렵다.
- 축제 연계는 전략 문구와 템플릿 수준이며, 실제 지역·월 이벤트를 확인해 Anchor로 고정하고 전후 관광·식음·숙박을 연결하는 단계는 미완료다.
- POI 큐레이션은 운영자 화면 없이 런타임 보조 계층으로만 유지한다. 큐레이션 테이블이 없는 환경에서는 기본 POI 조회로 폴백하며, 이번 문서 갱신에서 Neon DB에는 접근하지 않는다.

### 다음 작업 순서

1. **기획 입력 구조 고도화**: 8개 핵심 콘텐츠 테마(미식 / 자연 / 웰니스 / 문화·역사 / 문화예술 / 레저·액티비티 / K-콘텐츠 / 야간관광)를 고정 선택값으로 만들고, 반려동물·무장애·뚜벅이·가족을 별도 여행 조건으로 분리한다. 체류 확대·소비 확대·관광객 분산 목표도 분리한다.
2. **축제·이벤트 Anchor 연계**: 입력한 지역·여행월의 실제 이벤트를 확인할 수 있을 때만 축제를 고정 일정으로 넣고, 전후 관광·식음·숙박 후보를 연결한다. 데이터가 없으면 임의로 생성하지 않고 확인 필요로 표시한다.
3. **핵심 테마 슬롯과 후보 재정렬 강화**: 선택한 전략 테마가 코스에서 일정 비중을 차지하도록 후보 풀 점수·자동 초안·편집 후 재정렬을 한 흐름으로 묶는다.
4. **검증 품질 확장**: 구조화된 접근성·반려동물 근거를 활용해 검증하고, POI 유형별 권장 체류시간과 이동거리 억제를 적용한다. 외지인 방문 가치/대표성 평가는 장기 과제로 둔다.

> 현재 다음 개발 단위는 1번인 **“테마·여행 조건·관광 문제를 구조화하는 기획 입력 고도화”**다. 운영자 화면 구축이나 Neon DB 마이그레이션은 다음 작업에 포함하지 않는다.

# 구현 상태 (2026-08-18 갱신 — P1-2c 축제 Anchor 전후 후보 연결 반영)
...
| 3. 콘텐츠 테마 | 대부분 완료 | 미식 / 자연 / 웰니스 / 문화·역사 / 문화예술 / 레저·액티비티 / K-콘텐츠 / 야간관광 8개 고정 선택과 기존 자유 입력 자동 변환 | 핵심 테마 최소 비중과 코스 후보 재정렬을 전략별로 강화 |
| 4. 여행 조건 | 부분 완료 | 반려동물·무장애·뚜벅이·가족을 콘텐츠 테마와 분리해 저장·수정·분석 화면에 표시 | 실제 POI·숙박·식음 근거를 이용한 적합성 검증 |
...
| 7. 코스 스튜디오 | 완료 | 자동 초안, 우측 추천 후보 풀, Drag & Drop, 날짜 이동, 지도 마커·동선 갱신 | 핵심 테마 슬롯과 후보 재정렬을 더 강하게 연결 |
| 8. 실시간 검증 | 부분 완료 | 테마·이동·중복·식사·숙박·운영시간·실행 가능성 검증 패널 제공 | 무장애·반려동물 검증을 실제 데이터 근거와 함께 추가하고 장거리 이동 억제 강화 |
...
- 자동 초안·추천 후보 풀·Drag & Drop·지도·실시간 품질 패널은 이미 제품 흐름에 연결되어 있다. 좋은 후보 우선 추천과 동일 좌표 쇼핑 중복 억제도 내부 로직에 반영되어 있다.
...
- 여행 조건은 현재 저장·표시 단계까지다. 실제 POI·숙박·식음의 접근성·반려동물 근거를 확인하는 검증은 아직 남아 있다.
...
1. **핵심 테마 슬롯과 후보 재정렬 강화**: 선택한 전략 테마가 코스에서 일정 비중을 차지하도록 후보 풀 점수·자동 초안·편집 후 재정렬을 한 흐름으로 묶는다.
2. **검증 품질 확장**: 구조화된 접근성·반려동물 근거를 활용해 검증하고, POI 유형별 권장 체류시간과 이동거리 억제를 적용한다. 외지인 방문 가치/대표성 평가는 장기 과제로 둔다.
3. **축제 연계 품질 확장**: 축제 수혜 범위·상권 확산과 운영시간·접근성 근거를 후보 품질 신호로 추가한다.

> 이번 개발 단위에서 **“확정 Anchor의 명시적 코스 고정과 전후 후보 연결(P1-2b·P1-2c)”**을 로컬 코드·저장 검증·지도·실시간 검증·테스트까지 구현했다. 다음 개발 단위는 **“핵심 테마 슬롯과 후보 재정렬”**이며, Production Neon migration은 별도 승인 후 적용한다. 운영자 화면은 제품 범위에 포함하지 않는다.

...
**4-B) 비-음식 신 분류체계(lclsSystm1/2) 코드값 확인 및 POI 테마 적합도 활용(2026-08-14, POI 추천 품질
2차 고도화)** — 이전까지 "FOOD 관련 분류는 검증됐지만 비-FOOD lclsSystm1~3 의미는 검증되지 않았다"고
알려져 있었다. 이번에 위와 동일한 방식(`npm run verify:region -- --lcls-systm1 <코드> [--lcls-systm2
<코드>]`, `lclsSystmCode2` 오퍼레이션을 실 서비스키로 직접 호출)으로 실제 로컬 DB에 저장된 대분류
전부(NA/HS/VE/AC/LS/SH/EV/EX)의 공식 명칭을 확인했다.
- **HS(대분류 "역사관광", 확인됨)**: HS01 역사유적지/HS02 역사유물/HS03 종교성지/HS04 안보관광지.
- **NA(대분류 "자연관광", 확인됨)**: NA01~05 자연경관(산)/자연경관(하천·해양)/자연생태/자연공원/기타자연관광.
- **LS(대분류 "레포츠", 확인됨)**: LS01~04 육상/수상/항공/복합 레저스포츠.
- **VE(대분류 "문화관광", 확인됨)**: VE01~12(랜드마크관광/테마공원/도시공원/도시·지역문화관광/복합관광시설/
  공연시설/전시시설/행사시설/교육시설/레저스포츠시설/교통시설/기타문화관광지) — 테마파크(VE02)·레저스포츠
  시설(VE10) 등 문화·역사와 무관한 유형이 섞여 있어 대분류 전체는 신호로 쓰지 않는다. 중분류
  VE07(전시시설: 박물관/기념관/전시관/컨벤션센터/과학관/미술관)만 명확히 문화·역사와 연관된다.
- **EX(대분류 "체험관광", 확인됨)**: EX01~07(전통체험/공예체험/농산어촌체험/산사체험/웰니스관광/산업관광/
  기타체험). 중분류 EX05(웰니스관광: 온천/사우나/스파/찜질방/한방체험/힐링명상/뷰티스파/기타웰니스/
  자연치유/기타의료관광)가 WELLNESS 테마와 정확히 대응된다.
- **AC(대분류 "숙박", 확인됨)**: AC01~06 호텔/콘도미니엄/펜션·민박/모텔/캠핑/호스텔 — LODGING 카테고리
  매핑에 이미 쓰이고 있어 테마 신호로는 추가 활용하지 않는다.
- **SH(대분류 "쇼핑")**·**EV(대분류 "행사": 축제/공연/행사)**는 각각 SHOPPING/FESTIVAL PoiCategory와
  1:1로 대응돼 별도 테마 신호가 필요 없다.
- **실 로컬 DB 검증**: API로 동기화된 POI 48,268건 전부(100%)에 lclsSystm1이 저장돼 있다(카테고리별
  커버리지 동일하게 100%). 경주시 ATTRACTION 231건 중 124건(54%)이 "이름에 문화·역사 일반 키워드가
  전혀 없지만 lclsSystm1=HS 또는 lclsSystm2=VE07로 공식 확인되는" 사례였다(첨성대·대릉원·천마총 등 —
  이름에 "문화"/"역사"/"유적"/"고궁" 같은 단어가 없어 기존 이름 키워드 판정으로는 전혀 확인되지 않았다).
- **활용**: `src/lib/domain/audienceContext.ts`의 `classifyStructuralPoiThemes()`가 이 매핑을
  `computePoiFit()`(`src/lib/domain/poiFit.ts`)의 테마 적합도 판정에 이름 키워드보다 우선하는 신호로
  제공한다. 구조 신호가 없는 POI(FIXTURE, 매핑 없는 코드)는 기존 이름 키워드 판정으로 안전하게
  fallback한다. 상세 근거는 코드 주석과 `docs/implementation-status.md` 참고.
...
## 2026-08-08 지역 확장(Batch 3)에서 확인된 사항

- **`AreaTarDivService`(TOU_DIV_IX) 일일 호출 한도는 회복돼 있었다**: Batch 1+2 스크리닝 중 소진됐던
  이 한도가 이번 세션 시작 시점에는 정상이었다(기존 지역 1건 사전 확인 + 신규 후보 10건, 총 11회
  호출 — 429 없음). 다만 한도를 다시 소진할 위험을 줄이기 위해, 후보 코드 자체는 호출 한도가 없는
  TourAPI `ldongCode2`로 먼저 특정하고(시/도 목록 1회 + 관련 시/도별 시군구 목록 각 1회), `AreaTarDivService`는
  지역당 최종 확인 1회씩만 호출했다 — Batch 1+2가 후보 46곳을 전부 `areaTouDivList`로 스크리닝했던
  것과 달리, 이번에는 후보를 먼저 좁혀 이 오퍼레이션 호출 수를 최소화했다.
- **인천광역시·전북특별자치도의 시/도 코드가 통계청·TourAPI 두 체계에서 일치함을 확인했다**: 인천=28,
  전북특별자치도=52(2024년 특별자치도 전환 이후 코드) — 둘 다 `ldongCode2`(TourAPI)와 `areaTouDivList`
  (통계청) 양쪽에서 같은 번호로 응답해 §전남/광주 코드 불일치(TourAPI `ldongRegnCd=12`) 같은 사례가
  아님을 확인했다.
- **⚠️ Neon Postgres 프로젝트의 데이터 전송(data transfer) 쿼터 초과를 새로 발견했다**: 37개 지역 전체
  동기화(`npm run sync:tourism-data`) 실행 중 DB 연결이 `Your project has exceeded the data transfer
  quota. Upgrade your plan to increase limits.`로 완전히 끊겼다 — TourAPI/공공데이터 쪼터와는 무관한,
  Neon 플랜 자체의 한도다. 이후 최소 쿼리(`SELECT count(*)`)조차 동일하게 차단됨을 확인했다 — 재시도로
  해결되지 않는 계정/플랜 단위 한도로 보인다. 플랜 업그레이드 또는 다음 결제 주기 초기화 전까지는 DB를
  쓰는 모든 작업(동기화, 분석 생성, 앱 자체의 실제 사용)이 막힌다. 지역 확장처럼 여러 지역을 순차

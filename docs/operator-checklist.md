...
- [ ] 확인된 코드를 `src/lib/fixtures/regions.ts`의 `REGION_SEED`에 추가(SIDO가 새로 필요하면 SIDO 행도
      ...
      수집된다.
- [ ] `npm run db:seed` 실행(Region은 upsert라 기존 데이터에 안전) → 실 동기화(`npm run sync:tourism-data <baseYm>`,
      **위치 인자**임에 유의 — `--base-ym=` 같은 플래그 형식은 무시된 채 그대로 문자열로 들어가 전체
      동기화가 실패한다)로 해당 지역 지표/POI 확보 확인

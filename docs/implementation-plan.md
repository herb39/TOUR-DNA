...
| P0-4 | 대표 시나리오 3개(강릉/경주/제천) 결과 차별화 + E2E | 마스터 문서 2-3절이 요구하는 "입력 차이 → 결과 차이" 심사 시연의 직접 증거. P0-1~3이 실제로 동작하는지 검증하는 마지막 단계이자 발표력(15점) 대비 자료 | **로컬 구현+테스트 완료(새 P0-2로 승격, 완료) — 원격 반영·E2E 확장·실제 브라우저 검증 대기(2026-07-27)** |

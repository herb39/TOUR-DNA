...
입력한 지역과 여행월을 기준으로 실제 확인 가능한 이벤트만 탐색하고, 선택된 축제를 고정 일정으로 두어 전후 관광·식음·숙박 후보를 연결한다. 이벤트 데이터가 없거나 운영 여부를 확인할 수 없으면 임의의 축제를 추천하지 않고 확인 필요 상태로 표시한다.
...
> 따라서 현재 바로 시작할 작업은 **P0 기획 입력 구조 고도화**이며, 완료 후 P1 축제 Anchor 연계와 핵심 테마 슬롯 강화를 진행한다.

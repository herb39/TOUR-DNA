...
사용자가 선택한 Anchor를 프로젝트 단위로 안전하게 저장한 뒤, 명시적 확정 시에만 축제를 고정 일정으로 넣고 전후 관광·식음·숙박 후보를 연결한다. 자동 삽입, 기존 일정 순서 강제 변경, 전략 점수 변경은 하지 않는다. 저장 모델과 권한 범위는 구현 전에 별도 근거를 확인한다.
...
> P0 기획 입력 구조 고도화와 P1-1 축제 Anchor 후보 조회·선택은 완료됐다. 현재 바로 시작할 다음 작업은 **P1-2 선택 Anchor의 프로젝트 저장·코스 연결**이며, 이후 핵심 테마 슬롯과 후보 재정렬을 진행한다.

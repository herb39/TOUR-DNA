...
TOUR-DNA는 여행사·DMC, 지자체·관광재단 실무자를 위한 B2B 관광사업 의사결정 서비스입니다.
...
**주요 사용자**: 여행사/DMC 실무자, 지자체·관광재단 담당자
...
| 대표 시나리오 3개(강릉/경주/제천, P0-2) | 완료 | 완료(27개 파일 389/389) | 완료(push 완료) | 해당 없음(스키마 변경 없음) | **미배포** |
| 역할 적합 이유 노출 + 핵심 관광 지표 요약카드(2026-07-29~30) | 완료 | 완료(37개 파일 532/532) | 완료(push 완료) | 해당 없음(`Json` 필드 내 신규 키만 추가, 컬럼/스키마 변경 없음) | **확인 안 함**(이번 라운드에서 배포·DB 상태를 조회하지 않았음) |

실제 브라우저에서의 홍보자료 기능·대표 시나리오 카드 수동 검증은 아직 수행되지 않았다(홍보자료는
원격 DB에 migration을 적용하지 않고는 확인할 수 없어 보류 중, 대표 시나리오는 로컬에서도 아직 브라우저로
눌러보지 않았다) — "테스트 통과"와 "실제 배포·브라우저 검증 완료"를 같은 의미로 읽지 않는다. "GitHub
`main` 반영"은 로컬 git 이력과 원격 브랜치 상태 비교로만 확인한 사실이며, 운영 배포·DB 적용 여부와는
별개다.
...
- ⚠️ **로컬 저장소가 운영 배포보다 앞서 있을 수 있다**: `git log`/`git status` 기준으로는 로컬 `main`의
  모든 커밋이 `origin/main`에 push되어 있다(2026-07-30 확인). 다만 이는 GitHub 반영 여부만 확인한
  것이며, 홍보자료 기능(Phase 5)에 필요한 `20260726000000_add_selected_plan_promo_content` migration이
  운영 Neon DB에 적용됐는지, Vercel이 최신 커밋으로 실제 재배포됐는지는 이번 문서 갱신 작업에서
  DB·배포 상태를 조회하지 않아 별도 확인이 필요하다(순서는 [docs/deployment.md](docs/deployment.md)
  참고) — migration 없이 배포하면 해당 기능이 런타임 오류를 낸다.
...
|---:|---|---|---|---:|
| 1 | P0 | 전략별 POI 품질 개선 | 적합성 필터·추천 이유·중복 방지·후보 부족 경고 | 4~6인일 |
| 2 | P0 | 경주 대표 시나리오 통합 검증 | 생성→분석→전략→실행안→편집→홍보→인쇄 E2E | 2~3인일 |
| 3 | P0 | 유사지역 비교 | 관광 유형별 비교군·전국/유사지역 백분위 | 4~6인일 |
| 4 | P0 | 관광사업 기회발굴 | 취약점·계절·타깃·공급 격차 기반 사업 기회 3안 | 4~6인일 |
| 5 | P0 | 사업 사전검증 리포트 | 추진 권고·보완사항·위험·데이터 신뢰도 | 4~6인일 |
| 6 | P0 | KPI 연결 강화 | 사업 목적과 취약지표에 연결된 측정 KPI | 2~3인일 |
| 7 | P1 | 사업안 비교·예산·협력 대상 | 전략별 장단점·예산 항목·협업 기관 | 3~5인일 |
| 8 | P1 | 프로젝트별 접근 제어 | 접근키·비밀번호와 프로젝트 단위 보호 | 4~6인일 |
...
전국 확장은 전국 226개 지역을 시스템상 지원하되, 강릉·경주·제천과 유형별 대표 지역 약 20곳을
우선 검증합니다. 나머지는 데이터 품질에 따라 정상 분석·제한 분석·분석 불가로 구분하며, 품질이
부족하면 추천 결과를 임의 생성하지 않습니다.

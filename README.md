...
| 2. 코스 스튜디오(자동 초안 + 추천 후보 풀 패널 + 전문가 편집) | 현재 SelectedPlan/PlanEditor를 확장해 후보 POI를 별도 패널로 노출(이름/카테고리/theme fit/추천 근거/구조적 분류 근거/예상 이동 부담/권장 체류시간) | **부분 구현(2026-08-16)** — 추천 후보 풀 패널·Drag & Drop 편집은 완료(아래 3번 참고). 예상 이동거리 표시·저장 전 실시간 지도는 아직 없음(아래 8~9번 참고) |
...
| B. 코스 스튜디오 | 5) 추천 후보 풀, 6) 후보 추가/삭제, 7) Drag & Drop 재정렬/날짜 이동, 8) 지도 실시간 갱신, 9) 코스 실시간 품질검증 | **5~7번 완료(2026-08-16, 아래 상세 참고)**, 8~9번 미착수 |

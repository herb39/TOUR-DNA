...
- ⚠️ DNA 5축 점수는 같은 행정단위 코호트 안에서 min-max 정규화한다(scoring-model.md). 현재 SIGUNGU
  코호트가 대전/제천/양양 3개뿐이라, 3개 중 최댓값/최솟값에 해당하는 지역은 항상 정확히 100점/0점이
  된다(통계적으로 자연스러운 결과이지만, 표본이 3개뿐이라 체감상 "너무 자주 0 또는 100"으로 보일 수
  있다). 근본적인 개선은 비교 대상 지역을 늘리는 것이다 — 지역 확장 여부는 별도 논의 필요.

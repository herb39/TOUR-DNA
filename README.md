...
동작합니다("제품 링크 관리 > 웹 도메인"은 카카오톡 공유 링크용으로 별개 설정입니다).
...
| 지역별 관광 수요 강도 | 체류·소비 오퍼레이션 확인됐으나 모든 테스트 조합에서 데이터 0건 |
| 지역별 관광 자원 수요 / 방문자수 / 연관관광지 | 여전히 base URL·오퍼레이션명 미확인 |

⚠️ **다양성 지표 재계산 로직 미구현**: 지금은 확인된 단일 코드(`touDivIxCd=3103`, "30대 방문객수")를
그대로 저장하는데 이는 종합 다양성 점수가 아닙니다. 실 동기화(CLI/Cron)를 돌리면 fixture로 심어둔
다양성 점수가 의미가 다른 값으로 덮어써질 수 있습니다 — 자세한 내용과 대응은
[docs/operator-checklist.md](docs/operator-checklist.md) 참고.
...
- 지역별 관광 수요 강도(체류/소비는 오퍼레이션 확인, 수요는 미확인)·자원수요·방문자수·연관관광지 API는
  base URL·오퍼레이션명이 아직 불완전하거나 미확인이다. 다양성·국문관광정보는 실제 데이터로 확인됐다

...
| 홍보자료 생성·편집·저장·인쇄 출력(Phase 5, 5-A~5-C+보완) | 완료 | 완료(24개 파일 319/319) | **미반영**(로컬 전용 커밋 4개) | **미적용**(`20260726000000_add_selected_plan_promo_content`) | **미배포** |
| 역할·국적·테마·여행월의 분석 결과 실질 반영(Phase 4) | 완료 | 완료(25개 파일 361/361) | **미반영**(로컬 전용 커밋) | 해당 없음(스키마 변경 없음) | **미배포** |

실제 브라우저에서의 홍보자료 기능 수동 검증도 아직 수행되지 않았다(원격 DB에 migration을 적용하지
않고는 확인할 수 없어 보류 중) — "테스트 통과"와 "실제 배포·브라우저 검증 완료"를 같은 의미로 읽지 않는다.

## 목차

- [현재 구현 상태](#현재-구현-상태-2026-07-26-기준)
- [빠른 시작](#빠른-시작)
- [환경변수](#환경변수)
- [데이터베이스 준비](#데이터베이스-준비)
...
### 지금 순서대로 진행할 작업 (P0, 2026-07-26 갱신 — Phase 4 로컬 구현 완료 반영)

1. **P0-1. Phase 4 구현** — 역할·국적·테마·여행월이 표시용 값에 그치지 않고 실제 분석·전략·실행안
   결과에 반영되도록 한다(동일 입력 → 동일 결과가 나오는 결정적 구조 유지). **로컬 구현 및 자동
   테스트 완료**(원격 반영·DB 적용 대상 스키마 변경 없음·배포는 아직 미완료). 상세는
   [docs/implementation-status.md](docs/implementation-status.md)의 Phase 4 항목 참고.
2. **P0-2. 대표 시나리오 3개 완성** — 강릉·경주·제천(세 지역 모두 이미 시드 데이터 존재)을 서로 다른
   역할·국적·테마·여행월로 구성해 분석→전략→실행안→홍보자료→인쇄 전체 흐름을 시연 가능하게 만든다.
3. **P0-3. DB migration 적용 및 통합 검증** — Phase 5·시나리오 작업 이후, Neon DB 종류(개발/운영) 확인
   → 미적용 migration(`20260726000000_add_selected_plan_promo_content` 포함) 적용 → 홍보자료
   생성·편집·저장·재조회, 새로고침 유지, 재생성 취소/승인, 복사, 역할별 화면, 모바일, 인쇄, 기존 실행안
   회귀를 실제 DB·브라우저로 검증한다(Phase 4 조건별 차등화도 실제 화면에서 함께 확인).
4. **P0-4. 원격 반영 및 배포** — 통합 검증 후 `main` push → Vercel 빌드 확인 → 운영 환경변수·운영 DB
   migration 상태 확인 → 배포 URL smoke test.

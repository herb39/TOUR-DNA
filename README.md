...
| KPI 연결 강화(측정 목적·연결 DNA 축·연결 사업 목표·권장 시점·목표값 근거) | 완료 | 완료 | 미반영(이번 세션에서 로컬 구현만, 커밋 전) | 해당 없음(`Json` 필드 내 신규 키만 추가) | 미배포 |
| 전략 3안 비교(해결문제·활용자원·체류방식·실행난이도·기대효과·주요위험·적합역할) + 예산·협력 대상(CURATED) | 완료 | 완료 | 미반영(이번 세션에서 로컬 구현만, 커밋 전) | 해당 없음(기존 컬럼·Prisma 변경 없음) | 미배포 |
...
| ~~6~~ | ~~P0~~ | ~~사업 사전검증 리포트~~ | ~~추진 권고·보완사항·위험·데이터 신뢰도(DNA·POI·이동·유사지역 비교·위험 요인 조합)~~ | **완료(2026-08-03, 로컬+테스트, push 전)** |
| ~~7~~ | ~~P0~~ | ~~KPI 연결 강화~~ | ~~사업 목적과 취약지표에 연결된 측정 KPI(측정 목적·연결 DNA 축·연결 사업 목표·권장 시점·목표값 근거)~~ | **완료(2026-08-03~04, 로컬+테스트, push 전)** |
| ~~8~~ | ~~P1~~ | ~~사업안 비교·예산·협력 대상~~ | ~~전략별 장단점·예산 항목·협업 기관~~ | **완료(2026-08-04, 로컬+테스트, push 전)** |

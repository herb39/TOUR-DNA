...
아래는 2026-08-16 세션에서 합의한 중장기 제품 방향이다. 이번 세션에서 실제로 구현한 것은 "전략 핵심
테마 중심 코스 구성 강화"(아래 Phase A 1번) 하나뿐이며, 나머지는 전부 향후 검토·구현 후보다. 이미
구현된 기존 기능과 아이디어 단계 항목이 섞여 보이지 않도록, 각 항목에 **완료 / 부분 구현 / 데이터
조사 필요 / 장기 후보** 상태를 명시한다.
...
| 2. 코스 스튜디오(자동 초안 + 추천 후보 풀 패널 + 전문가 편집) | 현재 SelectedPlan/PlanEditor를 확장해 후보 POI를 별도 패널로 노출(이름/카테고리/theme fit/추천 근거/구조적 분류 근거/예상 이동 부담/권장 체류시간) | **부분 구현(2026-08-16)** — 추천 후보 풀 패널은 완료(이름/카테고리/적합도 배지/추천 근거, 날짜 선택 후 기존 장소 추가 흐름으로 코스에 반영). 예상 이동거리 표시·Drag & Drop·저장 전 실시간 지도는 아직 없음(아래 6~9번 참고) |
| 3. Drag & Drop 일정 편집 | 후보 풀→일정, 일정→다른 날짜, 동일 날짜 순서 변경을 Drag & Drop(모바일은 대체 UX)으로 | 장기 후보 — 현재는 위/아래 이동 버튼·날짜 이동·삭제·검색 추가만 지원(Drag & Drop 미지원, README "알려진 제한사항" 참고) |
| 4. 일정 편집 시 지도 실시간 갱신 | POI 추가/삭제/순서/날짜 변경 시 카카오맵 마커·동선·이동거리·이동시간 재계산 | 부분 구현 — 저장 후 새로고침하면 지도가 갱신되지만, 편집 중 실시간(저장 전) 갱신은 아직 없음 |
| 5. 실시간 코스 품질 검증 | 핵심 테마 POI 충분 여부·동일시설 중복·과도한 이동·식사/숙박 여부 등을 편집 중 또는 저장 직후 경고 | 부분 구현 — 저장된 실행안 기준 "사업 사전검증 리포트"(POI 공급 충분성·이동 현실성 등)는 있으나, 편집 중 실시간 검증은 없음 |
| 6. 콘텐츠 테마 8종(미식/자연/웰니스/문화·역사/문화예술/레저·액티비티/K-콘텐츠/야간관광) | 현재 `ThemeCategory` enum은 FOOD/NATURE/CULTURE_HISTORY/WELLNESS/FESTIVAL/PET_FRIENDLY/LEISURE_ACTIVITY 7종만 존재 | 미식/자연/웰니스/문화·역사: **완료**. 레저·액티비티: **부분 구현**(enum·EXPERIENCE 매핑은 있으나 LS 레포츠 세부 분류·권장 체류시간 미반영). 문화예술·K-콘텐츠·야간관광: **미구현**(enum 자체 없음) |
| 7. 레저·액티비티 세부 분류(수상·해양/겨울스포츠/아웃도어/익스트림/골프) | TourAPI 공식 `LS`(레포츠) 구조 신호를 세분화해 활용 | 데이터 조사 필요 — 현재는 `LS` 대분류만 EXPERIENCE에 매핑, 중분류 세분화는 미착수 |
...
| 10. 문화예술(미술관/전시/공연/현대문화공간) | TourAPI `VE07`(전시시설) 구조 신호 재사용 가능성 우선 검토 | 미구현 — `ThemeCategory` enum에 항목 자체가 없음, 공연/전시 운영시간 데이터 가용성 조사 필요 |
...
| 13. 축제·이벤트를 "기획 계기(Anchor Event)"로 재설계 | 지역+여행월에 실제 축제가 있으면 자동 탐색해 연계 전략 제안(오전 관광→식사→축제→야간상권→숙박→다음날 관광) | 부분 구현 — `FESTIVAL_EVENT` 전략 템플릿과 `ThemeCategory.FESTIVAL`은 있으나, 날짜/시간이 고정된 anchor로 취급하는 일정 구조는 없음(특정 축제 하드코딩 금지 원칙 유지) |
...
| B. 코스 스튜디오 | 5) 추천 후보 풀, 6) 후보 추가/삭제, 7) Drag & Drop 재정렬/날짜 이동, 8) 지도 실시간 갱신, 9) 코스 실시간 품질검증 | **5번 완료(2026-08-16, 아래 상세 참고), 6번 부분 구현(검색 추가/삭제는 기존에 있음)**, 7~9번 미착수 |
| C. 즉시 활용 가능한 콘텐츠 테마 확장 | 10) 문화예술, 11) 레저·액티비티 | 레저·액티비티 부분 구현, 문화예술 미구현 — 기존 TourAPI 공식 구조 데이터(`VE07`/`LS`) 우선 활용 예정 |
| D. 별도 데이터 검증이 필요한 확장 | 12) 축제·이벤트 연계, 13) 무장애 여행 조건, 14) K-콘텐츠, 15) 반려동물 동반, 16) 야간관광, 17) 뚜벅이/대중교통, 18) 가족/유아동 | 축제·이벤트만 부분 구현(전략 템플릿 수준), 나머지 데이터 가용성 확인 전까지 미착수 |
...
| 2 | P0 | 전략별 POI 품질 개선(추가 고도화) | 실제 도로/대중교통 경로 반영, 영업시간 기반 경고. **전략 핵심 테마 중심 코스 구성 강화는 2026-08-16 완료**(위 "TOUR-DNA 중장기 제품 로드맵" 1번 참고), 나머지(실제 경로/영업시간 경고)는 미착수 | 4~6인일 |

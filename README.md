...
자동 구성하는 B2B 웹 서비스입니다. 실행안이 확정되면 제안서 요약·랜딩페이지·SNS·블로그용 홍보자료도
역할(여행사/지자체)에 맞춰 생성·편집할 수 있습니다(**로컬 구현 완료, 아래 "현재 구현 상태" 참고 — 아직
운영 배포에는 반영되지 않았습니다**).

주요 사용자는 여행사/DMC 실무자와 지자체·관광재단 담당자다(`/projects/new`의 역할 선택값).
...
| 홍보자료 생성·편집·저장·인쇄 출력(Phase 5, 5-A~5-C+보완) | 완료 | 완료(24개 파일 319/319) | 완료(push 완료) | **미적용**(`20260726000000_add_selected_plan_promo_content`) | **미배포** |
| 역할·국적·테마·여행월의 분석 결과 실질 반영(Phase 4) | 완료 | 완료(25개 파일 361/361) | 완료(push 완료) | 해당 없음(스키마 변경 없음) | **미배포** |
| 대표 시나리오 3개(강릉/경주/제천, P0-2) | 완료 | 완료(27개 파일 389/389) | **미반영**(로컬 전용 커밋) | 해당 없음(스키마 변경 없음) | **미배포** |

실제 브라우저에서의 홍보자료 기능·대표 시나리오 카드 수동 검증은 아직 수행되지 않았다(홍보자료는
원격 DB에 migration을 적용하지 않고는 확인할 수 없어 보류 중, 대표 시나리오는 로컬에서도 아직 브라우저로
눌러보지 않았다) — "테스트 통과"와 "실제 배포·브라우저 검증 완료"를 같은 의미로 읽지 않는다.

## 목차

- [현재 구현 상태](#현재-구현-상태-2026-07-27-기준)
- [빠른 시작](#빠른-시작)
- [환경변수](#환경변수)
- [데이터베이스 준비](#데이터베이스-준비)
...
- **TourAPI 자체 한계**: 디저트·베이커리·주점을 위한 별도 코드가 없다 — 실제 데이터에서 "성심당"
  (베이커리)도 카페/전통찻집(`A05020900`) 하나로 들어오는 것을 확인했다. 이 이상의 세분화는 외부
  데이터 소스 구조상 불가능하다.
...
- ⚠️ **로컬 저장소가 운영 배포보다 앞서 있다**: 홍보자료 기능(Phase 5) 관련 커밋 4개가 로컬 `main`에만
  있고 `origin/main`에는 아직 push되지 않았다. push 전에 반드시 `20260726000000_add_selected_plan_promo_content`
  migration을 적용해야 한다(순서는 [docs/deployment.md](docs/deployment.md) 참고) — migration 없이 배포하면
  해당 기능이 런타임 오류를 낸다.

## 3분 시연 순서

> 아래는 현재 **운영 배포(https://tour-dna.lib.lc)** 기준 시연 순서다. 홍보자료 생성 기능은 로컬
> 구현은 끝났지만 아직 이 배포에는 포함되어 있지 않다.
...
### 지금 순서대로 진행할 작업 (P0, 2026-07-27 갱신 — 대표 시나리오 3개 로컬 구현 완료 반영)

1. **P0-1. Phase 4 구현 — 완료** — 역할·국적·테마·여행월이 표시용 값에 그치지 않고 실제 분석·전략·
   실행안 결과에 반영되도록 했다(동일 입력 → 동일 결과가 나오는 결정적 구조 유지). **로컬 구현·테스트,
   GitHub 반영(push) 완료**(DB 적용 대상 스키마 변경 없음·운영 배포는 아직 미완료). 상세는
   [docs/implementation-status.md](docs/implementation-status.md)의 Phase 4 항목 참고.
2. **P0-2. 대표 시나리오 3개 완성 — 완료(로컬)** — 강릉(여행사/외국인/미식·자연/8월)·경주(지자체/
   내국인/문화·역사/10월)·제천(여행사/외국인/웰니스/12월)을 `/projects/new`의 카드 프리셋으로 구현해
   기존 분석→전략→실행안 파이프라인을 그대로 통과시켰다. **로컬 구현·테스트 완료**(원격 반영·실제
   브라우저 검증은 아직). 상세는 [docs/implementation-status.md](docs/implementation-status.md)의
   "대표 시나리오 3개(P0-2)" 항목 참고.
3. **P0-3. DB migration 적용 및 통합 검증** — Neon DB 종류(개발/운영) 확인 → 미적용 migration
   (`20260726000000_add_selected_plan_promo_content` 포함) 적용 → 홍보자료 생성·편집·저장·재조회,
   새로고침 유지, 재생성 취소/승인, 복사, 역할별 화면, 모바일, 인쇄, 기존 실행안 회귀, 강릉·경주·제천
   대표 시나리오 3개를 실제 DB·브라우저로 검증한다.
4. **P0-4. 원격 반영 및 배포** — 통합 검증 후 `main` push(대표 시나리오 커밋 포함) → Vercel 빌드 확인
   → 운영 환경변수·운영 DB migration 상태 확인 → 배포 URL smoke test.

이 순서는 [docs/implementation-status.md](docs/implementation-status.md)의 "다음 작업 순서" 절과 동일하다.
아래 목록은 그 밖의 세부 개선 백로그다(우선순위 순, 앞부분일수록 임팩트가 크거나 지금 당장 필요).

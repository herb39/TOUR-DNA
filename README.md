...
| 2. 코스 스튜디오(자동 초안 + 추천 후보 풀 패널 + 전문가 편집) | 현재 SelectedPlan/PlanEditor를 확장해 후보 POI를 별도 패널로 노출(이름/카테고리/theme fit/추천 근거/구조적 분류 근거/예상 이동 부담/권장 체류시간) | 장기 후보 — 현재는 검색으로 등록 POI를 추가하는 기능만 있고 별도 추천 후보 풀 패널은 없음 |
...
| B. 코스 스튜디오 | 5) 추천 후보 풀, 6) 후보 추가/삭제, 7) Drag & Drop 재정렬/날짜 이동, 8) 지도 실시간 갱신, 9) 코스 실시간 품질검증 | 6번만 부분 구현(검색 추가/삭제는 있음), 나머지 미착수 |

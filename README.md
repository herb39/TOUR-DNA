...
- DNA 5축 점수는 같은 행정단위 코호트 안에서 min-max 정규화한다(scoring-model.md). SIGUNGU 코호트가
  ...
  화면에는 이 0/100이 절대평가가 아니라는 안내를 항상 노출하며, 반올림 때문에 0/100으로 보일 뿐
  실제 최저·최고가 아닌 경우(비교지역과 근소한 차이)는 "비교지역 내 최저/최고"가 아니라 "매우
  낮음/높음"으로 구분해 표시한다(2026-08-07 도입, `src/lib/domain/axisScoreInterpretation.ts`).

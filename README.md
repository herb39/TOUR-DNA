...
| 6. 콘텐츠 테마 8종(미식/자연/웰니스/문화·역사/문화예술/레저·액티비티/K-콘텐츠/야간관광) | 현재 `ThemeCategory` enum은 FOOD/NATURE/CULTURE_HISTORY/CULTURE_ARTS/WELLNESS/FESTIVAL/PET_FRIENDLY/LEISURE_ACTIVITY 8종 | 미식/자연/웰니스/문화·역사: **완료**. 문화예술: **부분 구현(2026-08-17 1차)** — 자유 입력 분류·`VE07` 공식 구조 신호·기존 문화·역사 전략 재사용까지 반영, 전용 전략은 후속. 레저·액티비티: **부분 구현(2026-08-17 1차)** — `LS01~04` 공식 중분류를 후보·일정 근거에 표시, 세부 체류시간 정책은 후속. K-콘텐츠·야간관광: **미구현** |
...
| C. 즉시 활용 가능한 콘텐츠 테마 확장 | 10) 문화예술, 11) 레저·액티비티 | **진행 중** — 문화예술은 분류·운영시간 데이터 연결까지 완료, 레저는 `LS01~04` 공식 중분류 표시까지 완료. 전용 전략·전국 배치·더 세밀한 체류시간 정책이 남음 |
